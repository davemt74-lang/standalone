(()=>{
  ...
  function renderMessage(row){
    ...
    if(row.attribution&&Number(row.attribution.attribution_count||0)>0){const lineage=document.createElement('div');lineage.className='agentResponseLineage';const info=document.createElement('span');const refs=Number(row.attribution.attribution_count||0),contributors=Number(row.attribution.contributor_count||0);info.textContent=refs+' Annotated reference'+(refs===1?'':'s')+' · '+contributors+' contributor'+(contributors===1?'':'s');lineage.appendChild(info);if(row.attribution.run_id){const link=document.createElement('a');link.href='/data-attribution.php?run_id='+encodeURIComponent(row.attribution.run_id);link.textContent='View lineage';lineage.appendChild(link);}article.appendChild(lineage);}
