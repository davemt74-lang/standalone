(()=>{
  ...
  document.addEventListener('annotated:agent-chat-request',e=>{const prompt=String(e.detail?.prompt||'').trim();const supplied=Array.isArray(e.detail?.context)?e.detail.context:[];if(supplied.length){selectedContext=supplied.slice(0,6).filter(x=>x&&x.type&&x.public_id).map(x=>({type:String(x.type),public_id:String(x.public_id),label:String(x.label||x.type)}));renderContextTray();}const eventConversation=String(e.detail?.conversation||'');const researchRequest=researchAgentMode||e.detail?.research_agent===true||eventConversation===requestedResearchAgentConversation||(activeConversation!==''&&activeConversation===requestedResearchAgentConversation);setModeAgent(researchRequest);if(eventConversation)activeConversation=eventConversation;if(prompt)sendPrompt(prompt);});
