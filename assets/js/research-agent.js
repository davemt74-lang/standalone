(()=>{
  ...
  function renderProposal(p){
    ...
    const details=document.createElement('details');details.className='agentActionProposalDetails';const detailsSummary=document.createElement('summary');detailsSummary.textContent='Review exact details';const dl=document.createElement('dl');Object.entries(p.arguments||{}).forEach(([k,v])=>{const dt=document.createElement('dt');dt.textContent=k.replace(/_/g,' ');const dd=document.createElement('dd');dd.textContent=Array.isArray(v)?v.join(', '):String(v??'');dl.append(dt,dd);});details.append(detailsSummary,dl);const provenance=document.createElement('small');provenance.className='agentActionProposalProvenance';const refCount=Array.isArray(p.provenance?.refs)?p.provenance.refs.length:0;provenance.textContent='Provenance: '+refCount+' attached Annotated reference'+(refCount===1?'':'s')+'.';
    ...
    const result=document.createElement('div');result.className='agentActionProposalResult';result.dataset.proposalResult='1';card.append(head,meta,summary,warning,details,provenance,actions,result);
  ...
  async function send(prompt){
    ...
    try{const d=await request('send',{conversation:conversation||null,prompt:prompt.trim(),context:[{type:'research',public_id:project}],client_message_id:client});conversation=d.conversation?.public_id||conversation;if(conversation)document.dispatchEvent(new CustomEvent('annotated:workspace-context',{detail:{research_public_id:project,agent_conversation_public_id:conversation,surface:'research'}}));try{sessionStorage.setItem(key,conversation);}catch{}thinking.remove();add('assistant',d.assistant_message?.body||'No response returned.',d.assistant_message?.action_proposals||[]);}
