...
function youtubeVideoId(){try{const u=new URL(location.href);if(u.hostname==='youtu.be')return u.pathname.slice(1).split('/')[0]||null;if(u.hostname.endsWith('youtube.com'))return u.searchParams.get('v')||null;}catch{}return null;}
function mediaInfo(){
...
function selectionPayload(includePageText=false){
  ...
  return {url:location.href,title:document.title,selectedText:text,selector,selectionRect,regionRect:lastRegion,pageText:includePageText?(document.body?.innerText||'').slice(0,150000):'',viewport:{width:innerWidth,height:innerHeight,devicePixelRatio:window.devicePixelRatio||1},...mediaInfo()};
}
