...
if($action==='presence'){
    ...
    $visible=[];$total=0;$following=0;foreach($q->fetchAll() as $r){if(is_blocked($pdo,(int)$u['id'],(int)$r['user_id']))continue;$total++;if($r['presence_mode']==='visible'){$visible[]=['public_id'=>$r['public_id'],'username'=>$r['username'],'display_name'=>$r['display_name'],'following'=>(bool)$r['is_following']];if($r['is_following'])$following++;}}
