...
if($action==='feed_page'||$action==='feed_following'){
    ...
    $rows=$q->fetchAll();foreach($rows as &$row){$row['screenshot_url']=!empty($row['screenshot_target_path'])?evidence_url((string)$row['public_id'],'target'):null;$row['audio_url']=!empty($row['audio_commentary_path'])?evidence_url((string)$row['public_id'],'audio'):null;$row['media_url']=!empty($row['media_path'])?evidence_url((string)$row['public_id'],'media'):null;unset($row['screenshot_target_path'],$row['audio_commentary_path'],$row['media_path']);}unset($row);json_response(['ok'=>true,'data'=>['annotations'=>$rows]]);
}
