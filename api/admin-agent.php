...
try{
    ...
    if($action==='send'){$admin=require_api_mutation_auth($pdo);admin_agent_assert_admin($pdo,$admin);$pageContext=is_array($input['page_context']??null)?$input['page_context']:[];$data=admin_agent_send($pdo,$config,$admin,trim((string)($input['thread']??'')),(string)($input['prompt']??''),isset($input['client_message_id'])?trim((string)$input['client_message_id']):null,$pageContext);json_response(['ok'=>true,'data'=>$data],201);}
