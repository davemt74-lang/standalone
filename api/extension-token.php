...
rate_limit_api($pdo,$config,'extension_token_ip',rate_limit_subject_ip(),30,600);
$raw=json_decode(file_get_contents('php://input'),true)?:[];
$code=(string)($raw['code']??'');$redirect=(string)($raw['redirect_uri']??'');
if($code!=='')rate_limit_api($pdo,$config,'extension_token_code','code:'.hash('sha256',$code),5,600);
