...
try{
    ...
    $version=mb_substr(trim((string)($raw['client_version']??'')),0,32)?:null;$device=mb_substr(trim((string)($raw['device_name']??'Chrome extension')),0,190);try{$q=$pdo->prepare('INSERT INTO extension_sessions(user_id,token_hash,device_name,client_version,expires_at) VALUES(?,?,?,?,?)');$q->execute([$row['user_id'],hash('sha256',$token),$device,$version,$expires]);}catch(PDOException $e){if(!str_contains(strtolower($e->getMessage()),'client_version'))throw $e;$q=$pdo->prepare('INSERT INTO extension_sessions(user_id,token_hash,device_name,expires_at) VALUES(?,?,?,?)');$q->execute([$row['user_id'],hash('sha256',$token),$device,$expires]);}
