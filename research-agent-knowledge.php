...
<title>Research Agent Knowledge · Annotated</title><link rel="stylesheet" href="/assets/css/app.css?v=64.0"></head>
...
<main class="researchLibraryCanvas researchKnowledgeCanvas">
  ...
  <section class="researchKnowledgeReports"><div class="sectionHeadWeb"><div><span class="eyebrow">SYSTEM REPORTS</span><h2>Processed views of this knowledge</h2></div><a class="button secondary" href="/research-reports.php?agent=<?=h(rawurlencode($selectedId))?>">All report types</a></div>
    <?php if(!$reports):?><div class="card empty">No System Reports have been generated yet.</div><?php endif?><div class="researchKnowledgeEvidenceGrid"><?php foreach($reports as $r):?><article class="card"><span class="eyebrow"><?=h(strtoupper((string)$r['type_label']))?></span><h3><?=h((string)$r['title'])?></h3><small><?=h((string)$r['created_at'])?> · Doc v<?=h((string)$r['document_revision'])?></small><a href="/home.php?agent=<?=h(rawurlencode($conversation))?>&doc=<?=h(rawurlencode((string)$r['document_public_id']))?>">Open report</a></article><?php endforeach?></div>
  </section>
  <?php endif?>
</main></body></html>