...
require dirname(__DIR__).'/app/bootstrap.php';release_worker_heartbeat($pdo,'transcription','starting','Worker invocation started.');
...
if(!$job){release_worker_heartbeat($pdo,'transcription','success','No queued transcription jobs.');echo "No queued transcription jobs.\n";exit(0);} $id=(int)$job['job_id'];$token=(string)$job['claim_token'];
...
try{
    ...
    if($command===''){
        $pdo->beginTransaction();job_claim_assert($pdo,'transcription_jobs',$id,$token);$pdo->prepare("UPDATE annotation_transcripts SET status='blocked',last_error=?,updated_at=NOW() WHERE id=?")->execute(['No transcription command configured.',$job['transcript_id']]);job_claim_complete($pdo,'transcription_jobs',$id,$token,'blocked');$pdo->commit();release_worker_heartbeat($pdo,'transcription','success','Job blocked: transcription command is not configured.',1);echo "Transcription blocked: configure transcription.command.\n";exit(0);
    }
    ...
    $q=$pdo->prepare('SELECT user_id,public_id FROM annotations WHERE id=?');$q->execute([$job['annotation_id']]);$a=$q->fetch();if($a){notify_user($pdo,(int)$a['user_id'],null,'transcript_ready','annotation',$a['public_id'],'Your audio commentary transcript is ready.');annotation_intelligence_queue($pdo,(int)$job['annotation_id'],(int)$a['user_id'],3);}
    release_worker_heartbeat($pdo,'transcription','success','Transcription completed.',1);echo "Transcription ready.\n";
}catch(LostJobClaim $e){if($pdo->inTransaction())$pdo->rollBack();if(isset($out))@unlink($out);release_worker_heartbeat($pdo,'transcription','failure','Lease lost; stale result discarded.');fwrite(STDERR,"Transcription lease lost; result discarded.\n");exit(2);}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();if(isset($out))@unlink($out);$msg=mb_substr($e->getMessage(),0,1000);try{$state=job_claim_retry_or_fail($pdo,'transcription_jobs',$id,$token,$msg,(int)$job['attempts'],3,120);$pdo->prepare("UPDATE annotation_transcripts SET status=?,last_error=?,updated_at=NOW() WHERE id=?")->execute([$state==='failed'?'failed':'queued',$msg,$job['transcript_id']]);}catch(LostJobClaim $lost){}release_worker_heartbeat($pdo,'transcription','failure',$msg);fwrite(STDERR,$msg."\n");exit(1);}
