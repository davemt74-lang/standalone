...
release_worker_heartbeat($pdo,'research-transcription','starting','Worker invocation started.');
$cfg=$config['transcription']??[];$command=trim((string)($cfg['command']??''));$provider=(string)($cfg['provider']??'local');$model=(string)($cfg['model']??'');
$job=job_claim($pdo,'research_transcription_jobs',"SELECT j.id job_id,j.*,t.object_id,rwo.public_id,rwo.created_by_user_id,rwo.title
  FROM research_transcription_jobs j JOIN research_workspace_recording_transcripts t ON t.id=j.transcript_id JOIN research_workspace_objects rwo ON rwo.id=t.object_id
  WHERE j.status='queued' AND j.available_at<=NOW() ORDER BY j.available_at,j.created_at LIMIT 1",[],7200);
if(!$job){release_worker_heartbeat($pdo,'research-transcription','success','No queued Research transcription jobs.');echo "No queued Research transcription jobs.\n";exit(0);}
...
try{
    ...
    if($command===''){
        ...
        release_worker_heartbeat($pdo,'research-transcription','success','Job blocked: transcription command is not configured.',1);echo "Research transcription blocked: configure transcription.command.\n";exit(0);
    }
    ...
    $text=trim((string)@file_get_contents($out));if($text==='')throw new RuntimeException('Transcription returned no text.');
    job_claim_renew($pdo,'research_transcription_jobs',$id,$token,7200);
    $pdo->beginTransaction();job_claim_assert($pdo,'research_transcription_jobs',$id,$token);
    $pdo->prepare("UPDATE research_workspace_recording_transcripts SET status='ready',raw_text=?,last_error=NULL,updated_at=NOW() WHERE id=?")->execute([$text,(int)$job['transcript_id']]);
    ...
    release_worker_heartbeat($pdo,'research-transcription','success','Research transcription completed.',1);echo "Research transcript ready.\n";
}catch(LostJobClaim $e){
    if($pdo->inTransaction())$pdo->rollBack();release_worker_heartbeat($pdo,'research-transcription','failure','Lease lost; stale result discarded.');fwrite(STDERR,"Research transcription lease lost.\n");exit(2);
}catch(Throwable $e){
    ...
    release_worker_heartbeat($pdo,'research-transcription','failure',$msg);fwrite(STDERR,$msg."\n");exit(1);
