...
$model=ai_setting_model_id($pdo,'admin',false);$profile=admin_ops_operator_profile($pdo,$admin);$from=trim((string)($_GET['from']??''));$pageContext=$from!==''?admin_agent_page_context_from_url($pdo,$admin,$from):null;
...
<main class="panel adminAgentPage" data-admin-agent-root data-api="/api/admin-agent.php" data-csrf="<?=h(csrf_token())?>" data-thread="<?=h((string)($active['public_id']??''))?>" data-context="<?=h(json_encode($pageContext,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE|JSON_THROW_ON_ERROR))?>">
  ...
  <div class="adminAgentWorkspace">
    <aside class="adminAgentThreads" aria-label="Admin Agent conversations">
      ...
      <?php foreach($threads as $thread):$selected=$active&&$active['public_id']===$thread['public_id'];?><a href="/admin/assistant.php?thread=<?=rawurlencode((string)$thread['public_id'])?><?= $from!==''?'&from='.rawurlencode($from):'' ?>" class="<?=$selected?'active':''?>" <?=$selected?'aria-current="page"':''?>><strong><?=h((string)($thread['title']?:'Admin Agent'))?></strong><?php if(!empty($thread['last_message'])):?><small><?=h((string)$thread['last_message'])?></small><?php endif?></a><?php endforeach?>
      </nav>
    </aside>
