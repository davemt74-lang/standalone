...
admin_access_assert_capability($pdo,$admin,'admin.finance.export');$type=trim((string)($_GET['type']??'ledger'));$filters=['from'=>(string)($_GET['from']??''),'to'=>(string)($_GET['to']??''),'status'=>(string)($_GET['status']??''),'account'=>(string)($_GET['account']??'')];
