...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if($action==='profile'){
        $display=trim((string)($_POST['display_name']??''));$bio=trim((string)($_POST['bio']??''));$website=trim((string)($_POST['website_url']??''));$image=trim((string)($_POST['profile_image_url']??''));
        ...
        if($image!==''){
            ...
            if(!filter_var($image,FILTER_VALIDATE_URL)||$scheme!=='https')$errors[]='Profile image must be a valid HTTPS image URL.';
        }
        if(!$errors){$pdo->prepare('UPDATE users SET display_name=?,bio=?,website_url=?,profile_image_url=? WHERE id=?')->execute([$display,$bio?:null,$website?:null,$image?:null,$u['id']]);$success='Profile updated.';}
    }
    if($action==='privacy'){
...
<header class="topbar"><a class="brand" href="/">Annotated</a><nav><a href="/home.php">Home</a><a href="/profile.php?u=<?=h($s['username'])?>">Profile</a><a href="/notifications.php">Notifications</a><a href="/connected-accounts.php">Accounts</a><a href="/logout.php">Sign out</a></nav></header>
...
<div class="card"><h2>Profile</h2><div class="profileMini" style="margin-bottom:18px"><?=app_shell_avatar($s,'avatarImageLg')?><span><strong><?=h($s['display_name'])?></strong><small>@<?=h($s['username'])?></small></span></div><form method="post" class="stack"><input type="hidden" name="csrf" value="<?=h(csrf_token())?>"><input type="hidden" name="action" value="profile"><label>Display name<input name="display_name" value="<?=h($s['display_name'])?>" required></label><label>Profile photo URL<input type="url" name="profile_image_url" value="<?=h((string)$s['profile_image_url'])?>" placeholder="https://…"></label><p class="meta">Use an HTTPS image URL. Your photo appears in the header, profile, Teams, feeds, and social activity.</p><label>Bio<textarea name="bio" rows="5" maxlength="2000"><?=h((string)$s['bio'])?></textarea></label><label>Website<input type="url" name="website_url" value="<?=h((string)$s['website_url'])?>"></label><button>Save profile</button></form></div>
