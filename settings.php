...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if($action==='profile'){
        ...
        if($display==='')$errors[]='Display name is required.';if($website!==''&&!filter_var($website,FILTER_VALIDATE_URL))$errors[]='Website URL is invalid.';
        if(!$errors){$pdo->prepare('UPDATE users SET display_name=?,bio=?,website_url=? WHERE id=?')->execute([$display,$bio?:null,$website?:null,$u['id']]);$success='Profile updated.';}
    }
    if($action==='privacy'){
...
<div class="card"><h2>Profile</h2><form method="post" class="stack"><input type="hidden" name="csrf" value="<?=h(csrf_token())?>"><input type="hidden" name="action" value="profile"><label>Display name<input name="display_name" value="<?=h($s['display_name'])?>" required></label><label>Bio<textarea name="bio" rows="5" maxlength="2000"><?=h((string)$s['bio'])?></textarea></label><label>Website<input type="url" name="website_url" value="<?=h((string)$s['website_url'])?>"></label><button>Save profile</button></form></div>
