...
try{
 ...
 if($provider==='google'){
  ...
  $userId=oauth_finish_login($pdo,'google',(string)$info['sub'],$info['email']??null,$info['name']??'Google User',isset($info['email'])?strstr($info['email'],'@',true):'google_user',(bool)($info['email_verified']??false));
 } elseif($provider==='x'){
  $verifier=(string)($_SESSION['oauth_verifier']??'');$auth=base64_encode($p['client_id'].':'.$p['client_secret']);
