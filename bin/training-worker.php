...
$max=max(1,min(50,(int)($argv[1]??5)));$done=0;$failed=0;release_worker_heartbeat($pdo,'training','starting','Training worker started.');
for($i=0;$i<$max;$i++){
...
$status=$failed>0?'failure':'success';release_worker_heartbeat($pdo,'training',$status,"Training worker complete · processed=$done · failures=$failed",$done);
