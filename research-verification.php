...
<?php if($c['human_review']['current']||$c['human_review']['stale']):?><details><summary>Human review history</summary><?php foreach($c['human_review']['current'] as $ev):?><p><strong><?=h((string)$ev['reviewer']['name'])?></strong> · <?=h(rv_label((string)$ev['decision']))?> · <?=h((string)$ev['created_at'])?><?php if($ev['note']):?><br><?=nl2br(h((string)$ev['note']))?><?php endif?></p><?php endforeach?><?php foreach($c['human_review']['stale'] as $ev):?><p class="meta"><strong><?=h((string)$ev['reviewer']['name'])?></strong> · prior <?=h(rv_label((string)$ev['decision']))?> · stale after Research/evidence changed · <?=h((string)$ev['created_at'])?></p><?php endforeach?></details><?php endif?>
