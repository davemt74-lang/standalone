...
$q=$pdo->prepare('UPDATE data_dataset_items SET normalized_text_snapshot=? WHERE dataset_id=? AND position=0');$q->execute(['tampered frozen text '.$run,$trainFrozen['id']]);$integrity=data_dataset_current_use_status($pdo,$trainable['public_id']);p38(!$integrity['usable']&&!$integrity['manifest_ok']&&$integrity['reason']==='manifest_integrity_failure','direct frozen-text tampering changes the recomputed manifest and is detected before dataset use');
