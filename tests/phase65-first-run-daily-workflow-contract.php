...
foreach(["['agent','workspace','doc']","function leaveResearchAgentToFeed","window.AnnotatedResearchWorkspace?.closeLibrary?.()","window.AnnotatedResearchWorkspace?.closeDesktop?.()","back?.addEventListener('click',leaveResearchAgentToFeed)","panelClose?.addEventListener('click',leaveResearchAgentToFeed)","setModeFeed();"] as $needle)if(!str_contains($chat,$needle))$fail[]='Research Agent safe return-to-Home behavior missing '.$needle;
if(str_contains($chat,"location.href='/research.php';return;}setModeFeed()"))$fail[]='Research Agent close must return to Home instead of redirecting to Research.';
