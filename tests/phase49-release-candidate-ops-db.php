...
$expectedLatestMigration='20260923_061_profile_public_identity_showcase.sql';
p49(release_latest_migration($root)===$expectedLatestMigration,'latest migration resolver identifies Profile Phase 2 migration 061');
p49(($release['latest_migration']??null)===$expectedLatestMigration,'release manifest identifies Profile Phase 2 migration 061 as current latest');
...
$specs=release_worker_specs();$expected=['media','transcription','research_files','research_transcription','research_retrieval','research_autonomy','research_monitor','research_tasks','research_programs','source_monitor','ai','saved_search','research_automation','evaluation','training','post_training'];
p49(array_keys($specs)===$expected,'one canonical release schedule covers all sixteen required workers');
...
$queues=release_queue_health($pdo);p49(isset($queues['research_files'],$queues['research_transcription'],$queues['research_retrieval'],$queues['research_autonomy'],$queues['research_monitor'],$queues['research_tasks'],$queues['evaluation'],$queues['training'],$queues['post_training']),'release queue health covers Research media plus Evaluation, Training, and Post-Training queues');
