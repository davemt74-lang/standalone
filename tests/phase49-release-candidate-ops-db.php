...
p49($release['latest_migration']==='20260923_054_continuous_research_monitoring.sql','release manifest identifies the current latest migration');
...
$specs=release_worker_specs();$expected=['media','transcription','research_files','research_transcription','research_retrieval','research_autonomy','research_monitor','source_monitor','ai','saved_search','research_automation','evaluation','training','post_training'];
p49(array_keys($specs)===$expected,'one canonical release schedule covers all fourteen required workers');
...
$queues=release_queue_health($pdo);p49(isset($queues['research_files'],$queues['research_transcription'],$queues['research_retrieval'],$queues['research_autonomy'],$queues['research_monitor'],$queues['evaluation'],$queues['training'],$queues['post_training']),'release queue health covers Research media plus Evaluation, Training, and Post-Training queues');
