...
p49($release['latest_migration']==='20260923_053_autonomous_research_workspace.sql','release manifest identifies the current latest migration');
...
$specs=release_worker_specs();$expected=['media','transcription','research_files','research_transcription','research_retrieval','research_autonomy','source_monitor','ai','saved_search','research_automation','evaluation','training','post_training'];
p49(array_keys($specs)===$expected,'one canonical release schedule covers all thirteen required workers');
...
$queues=release_queue_health($pdo);p49(isset($queues['research_files'],$queues['research_transcription'],$queues['research_retrieval'],$queues['research_autonomy'],$queues['evaluation'],$queues['training'],$queues['post_training']),'release queue health covers Research media plus Evaluation, Training, and Post-Training queues');
