...
$need('app/data-model-improvement.php','function data_model_improvement_actionable_proposal_locked','Phase 46 approval/publication must recheck current human triage while holding proposal + case authority.');
...
$updateBlock=$block($improvement,'data_model_improvement_proposal_update');if($updateBlock===''||!str_contains($updateBlock,'data_model_improvement_proposal_locked('))$fail[]='data_model_improvement_proposal_update must serialize proposal state.';
foreach(['data_model_improvement_proposal_approve','data_model_improvement_proposal_publish'] as $fn){$b=$block($improvement,$fn);if($b===''||!str_contains($b,'data_model_improvement_actionable_proposal_locked('))$fail[]="$fn must serialize proposal + source-case authority.";}
...
$need('app/data-model-campaigns.php','$checks[\'case_governance\']=','Campaign current-use validation must include the human-triage case snapshot.');
