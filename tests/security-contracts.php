...
$need('api/extension-pair.php','ORIGIN_MISMATCH','Pairing token exchange must bind the extension origin to the approved extension ID.');
$need('extension/sidepanel-state.js',"chrome.tabs.create({url})",'Extension Connect must use a normal browser tab for website approval.');
$need('extension/sidepanel-state.js',"/api/extension-pair.php",'Extension Connect must poll the dedicated pairing endpoint.');
$avoid('extension/sidepanel-state.js','launchWebAuthFlow','Extension Connect must not depend on Chrome launchWebAuthFlow.');
$avoid('extension/sidepanel-state.js','chrome.identity','Extension Connect must not depend on the Chrome identity API.');
...
$need('api/extension-pair.php','client_version','Extension pairing sessions must record the client version when available.');
...
$need('extension-connect.php','Chrome extension','Pairing approval must identify the Chrome extension to the user.');
$need('extension/options.js',"u.protocol!=='https:'",'Extension server settings must require HTTPS outside loopback development.');
$manifestSecurity=json_decode((string)file_get_contents($root.'/extension/manifest.json'),true);if(in_array('identity',$manifestSecurity['permissions']??[],true))$fail[]='Chrome identity permission must not be requested by the normal-tab pairing flow.';
