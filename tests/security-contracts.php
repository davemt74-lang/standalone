...
$phase11Init=(string)file_get_contents($root.'/extension/sidepanel-init.js');if(preg_match("/(?<!\\$)\\$\\('nav button'\\)\\.forEach/",$phase11Init))$fail[]='Sidebar nav binding must use querySelectorAll helper.';if(preg_match("/(?<!\\$)\\$\\('\\.modes button'\\)\\.forEach/",$phase11Init))$fail[]='Capture mode binding must use querySelectorAll helper.';
