...
try{
    ...
    installer_run($pdo,$root.'/database/schema.sql',$tmp);if(!installer_table_exists($pdo,'stripe_webhook_events'))throw new RuntimeException('065 fixture did not reach Stripe Billing.');if(installer_table_exists($pdo,'stripe_account_state_watermarks'))throw new RuntimeException('065 fixture unexpectedly contains audit-hardening watermark schema.');
    $applied=migration_apply_pending($pdo,$root.'/database/migrations',10);if(!in_array('20260924_066_code_audit_hardening',$applied,true))throw new RuntimeException('Upgrade did not apply migration 066.');if(!installer_table_exists($pdo,'stripe_account_state_watermarks'))throw new RuntimeException('066 upgraded schema missing stripe_account_state_watermarks.');
