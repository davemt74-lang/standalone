...
$token=bin2hex(random_bytes(32));$pdo->prepare("INSERT INTO extension_sessions(user_id,token_hash,device_name,client_version,expires_at) VALUES(?,?,?,'0.36.0',DATE_ADD(NOW(),INTERVAL 30 DAY))")->execute([$user['id'],hash('sha256',$token),'CI Chrome']);
...
$config=['app'=>['base_url'=>'https://annotated.example.test','encryption_key'=>str_repeat('a',40)],'storage'=>['private_root'=>$tmp],'extension'=>['allowed_ids'=>['abcdefghijklmnopabcdefghijklmnop']],'oauth'=>['google'=>['client_id'=>'x','client_secret'=>'y','redirect_uri'=>'https://annotated.example.test/oauth/callback.php'],'x'=>['client_id'=>'x','client_secret'=>'y','redirect_uri'=>'https://annotated.example.test/oauth/callback.php']],'transcription'=>['command'=>'/bin/true']];
$health=release_environment_checks($pdo,$config);p11(($health['checks']['database']['status']??'')==='pass','release preflight validates database connection');p11(($health['checks']['migrations']['status']??'')==='pass','release preflight confirms all migrations applied');p11(($health['checks']['extension_ids']['status']??'')==='pass','release preflight validates extension allowlist');p11(($health['checks']['worker_media']['status']??'')==='fail'&&!$health['ready'],'failed required worker blocks RC readiness');p11($health['release']===ANNOTATED_RELEASE&&$health['version']===ANNOTATED_RELEASE_VERSION&&$health['extension_version']===ANNOTATED_EXTENSION_VERSION,'release health reports exact RC identity');@rmdir($tmp);

$q=$pdo->prepare('SELECT client_version,expires_at FROM extension_sessions WHERE user_id=? ORDER BY id DESC LIMIT 1');$q->execute([$user['id']]);$session=$q->fetch();p11(($session['client_version']??'')===ANNOTATED_EXTENSION_VERSION&&!empty($session['expires_at']),'extension session records current RC client version and expiry');
