...
$q=$pdo->query("SELECT COUNT(*) FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='media_jobs' AND column_name='lease_token'");if((int)$q->fetchColumn()!==1)throw new RuntimeException('Worker lease migration was not applied.');
