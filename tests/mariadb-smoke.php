...
$files=glob($root.'/database/migrations/*.sql')?:[];sort($files,SORT_STRING);$applied=migration_apply_pending($pdo,$root.'/database/migrations');
if(count($applied)!==count($files))throw new RuntimeException('Production migration runner did not apply every migration to the fresh schema.');
if((int)$pdo->query('SELECT COUNT(*) FROM schema_migrations')->fetchColumn()!==count($files))throw new RuntimeException('Migration ledger count does not match migration files.');
...
echo 'MariaDB schema, production migration runner, and rate limiting smoke test passed ('.count($files)." migrations).\n";
