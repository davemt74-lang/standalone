...
foreach(['database/migrations/20260921_036_data_attribution_v1.sql','app/data-attribution.php','api/data-attribution.php','data-attribution.php','admin/data-attribution.php','bin/data-attribution-sync.php','tests/phase37-data-attribution-db.php','tests/phase37-data-attribution-contract.php'] as $file)if(!is_file($root.'/'.$file))$fail[]='Phase 37 file missing: '.$file;
...
foreach(["'reason'=>'object_not_public'","no_contributor_consent","'reason'=>'source_rights_not_approved'"] as $needle)if(!str_contains($runtime,$needle))$fail[]='Eligibility boundary missing: '.$needle;
...
$need('home.php','agent-chat.js?v=38.0','Phase 37+ Agent Chat client must retain a fresh cache key after the Home feed/Research Agent split.');
...
$need('app/agent-actions.php','data_provenance_try_edge_record','Confirmed Agent Claim relationships must enter the shared provenance graph.');
