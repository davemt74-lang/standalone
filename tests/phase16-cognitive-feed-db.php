...
$orphanUrl='https://orphan.example.com/'.$run;$orphanPublic=$pub('source');$pdo->prepare("INSERT INTO sources(public_id,source_type,canonical_url,canonical_url_hash,domain,title,status,monitoring_enabled,moderation_status) VALUES(?,'webpage',?,?,?,'Orphan Watched Source','updated',1,'visible')")->execute([$orphanPublic,$orphanUrl,hash('sha256',$orphanUrl),'orphan.example.com']);$orphanId=(int)$pdo->lastInsertId();$pdo->prepare("INSERT INTO source_versions(source_id,version_number,final_url,title,extracted_text,content_hash) VALUES(?,1,?,'Orphan','Orphan text',?)")->execute([$orphanId,$orphanUrl,hash('sha256','orphan')]);$orphanV=(int)$pdo->lastInsertId();$pdo->prepare('UPDATE sources SET current_version_id=? WHERE id=?')->execute([$orphanV,$orphanId]);$pdo->prepare('INSERT INTO source_watches(source_id,user_id) VALUES(?,?)')->execute([$orphanId,$owner['id']]);$pdo->prepare("INSERT INTO source_change_events(source_id,new_version_id,change_type,impact_type,target_changed,diff_summary) VALUES(?,?,'updated','source_updated',0,'Orphan watched source change')")->execute([$orphanId,$orphanV]);
$orphanFeed=cognitive_feed_compose($pdo,$owner,null,8,60);p16(count(array_filter(p16find($orphanFeed,'source_change'),fn($x)=>str_contains((string)($x['title']??''),'Orphan Watched Source')))===0,'watched-source history cannot bypass current source_access permissions');
