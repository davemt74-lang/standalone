...
$conversationReady=conversation_runtime_ready($pdo);$chatTeams=$conversationReady?conversation_team_list($pdo,$u):[];$preferredTeam=trim((string)($_GET['team']??''));$preferredTeamContext=$preferredTeam!==''&&in_array($preferredTeam,array_column($chatTeams,'team_public_id'),true)?$preferredTeam:'';$chatStatus=conversation_presence_ready($pdo)?conversation_status_get($pdo,(int)$u['id']):['status_mode'=>'auto','custom_status'=>'','effective_status'=>'offline'];
