...
function billing_operations_current_metrics(PDO $pdo,?string $mode=null): array {
    ...
    foreach(billing_operations_recurring_rows($pdo,$mode,true) as $row){if(empty($row['billing_visible']))continue;$source=(string)$row['billing_source'];if(!in_array($source,['stripe','manual'],true))continue;$status=(string)$row['effective_subscription_status'];if(isset($counts[$status]))$counts[$status]++;$mrr+=billing_operations_row_mrr($row);}
...
function billing_operations_revenue_by_package(PDO $pdo,?string $mode=null): array {
    ...
    foreach(billing_operations_recurring_rows($pdo,$mode,true) as $row){if(empty($row['billing_visible']))continue;$pid=(int)$row['package_id'];if(!isset($out[$pid]))continue;$status=(string)$row['effective_subscription_status'];$mrr=billing_operations_row_mrr($row);if($mrr>0){$out[$pid]['billable_accounts']++;$out[$pid]['mrr_cents']+=$mrr;}if($status==='trialing')$out[$pid]['trialing_accounts']++;if($status==='past_due')$out[$pid]['past_due_accounts']++;}
