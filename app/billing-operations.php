...
function billing_operations_lifecycle_superseded(PDO $pdo,array $case): bool {
    $accountId=(int)($case['account_id']??0);if($accountId<=0)return true;$marker=(int)($case['suspension_lifecycle_event_id']??0);
    if($marker>0){$q=$pdo->prepare("SELECT COUNT(*) FROM account_admin_events WHERE account_id=? AND event_type='lifecycle_changed' AND id>?");$q->execute([$accountId,$marker]);return (int)$q->fetchColumn()>0;}
    if(!empty($case['suspended_at'])){$q=$pdo->prepare("SELECT COUNT(*) FROM account_admin_events WHERE account_id=? AND event_type='lifecycle_changed' AND created_at>? AND reason<>'Billing dunning grace expired.'");$q->execute([$accountId,(string)$case['suspended_at']]);return (int)$q->fetchColumn()>0;}
    return false;
}
    ...
    commercial_account_with_lock($pdo,(int)$account['id'],function()use($pdo,$object,$eventType,$mode,$eventId,$eventCreatedAt,$invoiceId,$account){
        ...
        if(in_array($eventType,['invoice.payment_failed','invoice.payment_action_required'],true)){
            ...
            $restored=false;if((int)$existing['suspended_by_dunning']===1&&!empty($settings['restore_after_payment'])&&!empty($existing['suspended_at'])){
                ...
                $laterLifecycle=billing_operations_lifecycle_superseded($pdo,$existing);
                if($fresh&&$fresh['status']==='suspended'&&$other===0&&!$laterLifecycle){$pdo->prepare("UPDATE accounts SET status='active' WHERE id=? AND status='suspended'")->execute([(int)$account['id']]);$restored=true;}
            }
...
function billing_operations_process_due_dunning(PDO $pdo,?array $admin=null): array {
    ...
    foreach($rows as $row){commercial_account_with_lock($pdo,(int)$row['account_id'],function()use($pdo,$row,$settings,$admin,&$reviewed,&$suspended,&$grace){
        ...
        if(!empty($settings['suspend_after_grace'])&&$account['status']==='active'){
            $pdo->prepare("UPDATE accounts SET status='suspended' WHERE id=? AND status='active'")->execute([(int)$account['id']]);$pdo->prepare("UPDATE billing_dunning_cases SET status='suspended',stage=2,suspended_by_dunning=1,suspended_at=NOW(),suspension_lifecycle_event_id=NULL,next_review_at=NULL WHERE id=?")->execute([(int)$case['id']]);billing_operations_dunning_event($pdo,(int)$case['id'],(int)$account['id'],$admin?'admin':'system','dunning_suspended','Dunning grace expired and the configured policy suspended account access.',$admin?(int)$admin['id']:null,null,$before,['status'=>'suspended','account_status'=>'suspended']);if(function_exists('account_admin_event')){account_admin_event($pdo,(int)$account['id'],$admin?(int)$admin['id']:null,null,'lifecycle_changed',['status'=>'active'],['status'=>'suspended'],'Billing dunning grace expired.');$lifecycleEventId=(int)$pdo->lastInsertId();if($lifecycleEventId>0)$pdo->prepare("UPDATE billing_dunning_cases SET suspension_lifecycle_event_id=? WHERE id=?")->execute([$lifecycleEventId,(int)$case['id']]);}$suspended++;
        }else{
...
function billing_operations_grant_grace(PDO $pdo,array $admin,string $casePublicId,int $days,string $reason): array {
    ...
    return commercial_account_with_lock($pdo,(int)$seed['account_id'],function()use($pdo,$admin,$seed,$days,$reason){
        ...
        if($case['status']==='suspended'&&(int)$case['suspended_by_dunning']===1&&$account['status']==='suspended'&&!billing_operations_lifecycle_superseded($pdo,$case)){$pdo->prepare("UPDATE accounts SET status='active' WHERE id=?")->execute([(int)$account['id']]);$restored=true;}
