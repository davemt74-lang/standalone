...
function billing_operations_handle_invoice_event(PDO $pdo,array $object,string $eventType,string $mode,?string $eventId,?string $eventCreatedAt): void {
    ...
    commercial_account_with_lock($pdo,(int)$account['id'],function()use($pdo,$object,$eventType,$mode,$eventId,$eventCreatedAt,$invoiceId,$account){
        ...
        $amountDue=(int)($object['amount_due']??0);$remaining=(int)($object['amount_remaining']??0);
        if(in_array($eventType,['invoice.payment_failed','invoice.payment_action_required'],true)){
            ...
            $restored=false;if((int)$existing['suspended_by_dunning']===1&&!empty($settings['restore_after_payment'])&&!empty($existing['suspended_at'])){
                ...
                $accountUpdated=!empty($fresh['updated_at'])?strtotime((string)$fresh['updated_at']):0;$suspendedAt=strtotime((string)$existing['suspended_at']);
                if($fresh&&$fresh['status']==='suspended'&&$other===0&&$accountUpdated<=$suspendedAt+5){$pdo->prepare("UPDATE accounts SET status='active' WHERE id=? AND status='suspended'")->execute([(int)$account['id']]);$restored=true;}
            }
...
function billing_operations_process_due_dunning(PDO $pdo,?array $admin=null): array {
    ...
    foreach($rows as $row){commercial_account_with_lock($pdo,(int)$row['account_id'],function()use($pdo,$row,$settings,$admin,&$reviewed,&$suspended,&$grace){
        $q=$pdo->prepare('SELECT * FROM billing_dunning_cases WHERE id=? FOR UPDATE');$q->execute([(int)$row['id']]);$case=$q->fetch();if(!$case||!in_array($case['status'],['open','action_required','grace'],true))return;$reviewed++;
        $account=account_admin_get($pdo,(int)$case['account_id']);if(!$account)return;$before=['status'=>$case['status'],'account_status'=>$account['status'],'grace_until'=>$case['grace_until']];
        if(!empty($settings['suspend_after_grace'])&&$account['status']==='active'){
            $pdo->beginTransaction();try{$pdo->prepare("UPDATE accounts SET status='suspended' WHERE id=? AND status='active'")->execute([(int)$account['id']]);$pdo->prepare("UPDATE billing_dunning_cases SET status='suspended',stage=2,suspended_by_dunning=1,suspended_at=NOW(),next_review_at=NULL WHERE id=?")->execute([(int)$case['id']]);billing_operations_dunning_event($pdo,(int)$case['id'],(int)$account['id'],$admin?'admin':'system','dunning_suspended','Dunning grace expired and the configured policy suspended account access.',$admin?(int)$admin['id']:null,null,$before,['status'=>'suspended','account_status'=>'suspended']);if(function_exists('account_admin_event'))account_admin_event($pdo,(int)$account['id'],$admin?(int)$admin['id']:null,null,'lifecycle_changed',['status'=>'active'],['status'=>'suspended'],'Billing dunning grace expired.');$pdo->commit();$suspended++;}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
        }else{
...
function billing_operations_grant_grace(PDO $pdo,array $admin,string $casePublicId,int $days,string $reason): array {
    ...
    return commercial_account_with_lock($pdo,(int)$seed['account_id'],function()use($pdo,$admin,$seed,$days,$reason){
        $q=$pdo->prepare('SELECT * FROM billing_dunning_cases WHERE id=? FOR UPDATE');$q->execute([(int)$seed['id']]);$case=$q->fetch()?:throw new RuntimeException('Dunning case not found.');if(in_array($case['status'],['recovered','uncollectible','closed'],true))throw new RuntimeException('Closed dunning cases cannot receive grace.');
        ...
        $pdo->beginTransaction();try{if($case['status']==='suspended'&&(int)$case['suspended_by_dunning']===1&&$account['status']==='suspended'){$pdo->prepare("UPDATE accounts SET status='active' WHERE id=?")->execute([(int)$account['id']]);$restored=true;}
...
function billing_operations_snapshot_day(PDO $pdo,?string $date=null,?string $mode=null): array {
    ...
    $new=$expansion=$contraction=0;$q=$pdo->prepare("SELECT e.*,pp.monthly_price_cents previous_price,np.monthly_price_cents new_price FROM subscription_package_events e LEFT JOIN subscription_packages pp ON pp.id=e.previous_package_id LEFT JOIN subscription_packages np ON np.id=e.new_package_id WHERE e.created_at>=? AND e.created_at<? ORDER BY e.id");$q->execute([$start,$end]);foreach($q->fetchAll()?:[] as $e){$old=(int)($e['previous_price']??0);$next=(int)($e['new_price']??0);if($next>$old){if($old===0)$new+=$next;else $expansion+=($next-$old);}elseif($old>$next&&$next>0)$contraction+=($old-$next);}
    $gross=$failed=$refunded=$disputed=$recovered=0;$churn=0;$churnAccounts=[];$q=$pdo->prepare("SELECT e.*,a.package_id FROM account_billing_events e JOIN accounts a ON a.id=e.account_id WHERE e.created_at>=? AND e.created_at<? ORDER BY e.id");$q->execute([$start,$end]);foreach($q->fetchAll()?:[] as $e){$before=json_decode((string)($e['before_json']??''),true)?:[];$after=json_decode((string)($e['after_json']??''),true)?:[];$type=(string)$e['event_type'];if($type==='invoice_paid'){$gross+=(int)($after['amount_paid_cents']??0);if(($before['subscription_status']??'')==='past_due')$recovered+=billing_operations_price($pdo,(int)$e['package_id']);}elseif(in_array($type,['invoice_payment_failed','invoice_payment_action_required'],true))$failed+=(int)($before['amount_due_cents']??0);elseif($type==='charge_refunded')$refunded+=(int)($after['amount_cents']??0);elseif($type==='charge_dispute_created')$disputed+=(int)($after['amount_cents']??0);elseif($type==='subscription_canceled'&&!isset($churnAccounts[(int)$e['account_id']])){$churn+=billing_operations_price($pdo,(int)$e['package_id']);$churnAccounts[(int)$e['account_id']]=true;}}
