...
function data_model_release_decision_integrity(PDO $pdo,array $decision): array {
    if($decision['status']!=='decision_recorded'||empty($decision['decision_hash'])||empty($decision['final_signature_hash'])||empty($decision['final_signed_at'])||empty($decision['final_signed_by_user_id']))return ['ok'=>false,'reason'=>'final_decision_unavailable'];$material=data_model_release_final_hash_material($pdo,$decision,(string)$decision['outcome'],(string)$decision['rationale'],(string)$decision['final_signed_at'],(int)$decision['final_signed_by_user_id']);$computed=data_attribution_hash($material);$sig=data_attribution_hash(['decision_hash'=>$computed,'signer_user_id'=>(int)$decision['final_signed_by_user_id'],'signed_at'=>$decision['final_signed_at'],'outcome'=>$decision['outcome']]);$ok=hash_equals((string)$decision['decision_hash'],$computed)&&hash_equals((string)$decision['final_signature_hash'],$sig);return ['ok'=>$ok,'reason'=>$ok?'current':'decision_integrity_failure','stored_hash'=>$decision['decision_hash'],'computed_hash'=>$computed,'stored_signature'=>$decision['final_signature_hash'],'computed_signature'=>$sig];
}
