...
function oauth_finish_login(PDO $pdo,string $provider,string $providerId,?string $email,string $displayName,string $usernameHint,bool $emailVerified,?int $linkUserId=null): int {
    ...
    try{$q=$pdo->prepare('INSERT INTO users(public_id,username,display_name,email,email_verified_at,password_hash) VALUES(?,?,?,?,?,NULL)');$q->execute([$public,$username,$displayName?:$username,$emailVerified?$email:null,$emailVerified?date('Y-m-d H:i:s'):null]);$uid=(int)$pdo->lastInsertId();$q=$pdo->prepare('INSERT INTO user_identities(user_id,provider,provider_user_id,provider_email) VALUES(?,?,?,?)');$q->execute([$uid,$provider,$providerId,$email]);$pdo->commit();if(function_exists('subscription_ensure_user_account'))subscription_ensure_user_account($pdo,$uid);if(function_exists('onboarding_ensure'))onboarding_ensure($pdo,$uid);return $uid;}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
}
