...
function ai_research_context(PDO $pdo,int $projectId): array {
    ...
    $q=$pdo->prepare("SELECT a.public_id,a.text_commentary,c.selected_text,COALESCE(at.edited_text,at.raw_text) transcript_text FROM project_annotations pa JOIN annotations a ON a.id=pa.annotation_id JOIN captures c ON c.id=a.capture_id LEFT JOIN annotation_transcripts at ON at.annotation_id=a.id WHERE pa.project_id=? ORDER BY pa.created_at DESC LIMIT 30");$q->execute([$projectId]);foreach($q->fetchAll() as $r){$refs[]=['type'=>'annotation','id'=>$r['public_id']];$chunks[]='[ANNOTATION '.$r['public_id']."]\nCommentary: ".mb_substr((string)$r['text_commentary'],0,2500)."\nCaptured: ".mb_substr((string)$r['selected_text'],0,3000).($r['transcript_text']?"\nTranscript: ".mb_substr((string)$r['transcript_text'],0,3000):'');}
