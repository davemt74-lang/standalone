...
function search_unified(PDO $pdo,string $rawTerm,?array $viewer,array $rawFilters=[],bool $recordRecent=true): array {
    ...
    foreach($q->fetchAll() as $s){
        ...
        [$ca,$cp]=feed_access_sql($viewer,'aa');$cq=$pdo->prepare("SELECT COUNT(*) FROM annotations aa WHERE aa.source_id=? AND $ca");$cq->execute(array_merge([$s['id']],$cp));$s['annotation_count']=(int)$cq->fetchColumn();$sources[]=$s;if(count($sources)>=24)break;
    }
    ...
    $q=$pdo->prepare("SELECT u.public_id,u.username,u.display_name,u.bio,u.profile_image_url FROM users u LEFT JOIN user_preferences p ON p.user_id=u.id WHERE u.status='active' AND COALESCE(p.profile_visibility,'public')='public' AND COALESCE(p.search_visibility,1)=1 AND (u.username LIKE ? OR u.display_name LIKE ? OR u.bio LIKE ? OR SOUNDEX(u.display_name)=SOUNDEX(?))".$userBlock." ORDER BY CASE WHEN u.username=? THEN 0 WHEN u.display_name LIKE ? THEN 1 ELSE 2 END,u.display_name LIMIT 24");$q->execute([$like,$like,$like,$term,$term,$prefix]);$people=$q->fetchAll();
...
function search_entity_results(PDO $pdo,string $term,?array $viewer,?array $project=null,int $limit=30): array {
    $like='%'.$term.'%';$entities=[];
    try{$q=$pdo->prepare("SELECT de.public_id,de.entity_type,de.canonical_name,de.description,COUNT(dem.object_public_id) mention_count,MAX(dem.updated_at) last_activity FROM discovery_entities de LEFT JOIN discovery_entity_aliases da ON da.entity_id=de.id LEFT JOIN discovery_entity_mentions dem ON dem.entity_id=de.id WHERE de.status='active' AND (de.canonical_name LIKE ? OR de.description LIKE ? OR da.alias_name LIKE ? OR SOUNDEX(de.canonical_name)=SOUNDEX(?)) GROUP BY de.id ORDER BY mention_count DESC,de.canonical_name LIMIT ".$limit);$q->execute([$like,$like,$like,$term]);foreach($q->fetchAll() as $e){$e['scope']='public';$entities[]=$e;}}catch(PDOException $e){}
...
function search_recent(PDO $pdo,array $viewer,int $limit=10): array {
    $q=$pdo->prepare('SELECT query_text,filters_json,use_count,last_used_at FROM search_recent_queries WHERE user_id=? ORDER BY last_used_at DESC LIMIT '.max(1,min(20,$limit)));$q->execute([$viewer['id']]);$rows=$q->fetchAll();foreach($rows as &$r)$r['filters']=json_decode((string)$r['filters_json'],true)?:[];unset($r);return $rows;
}
function search_saved_access(PDO $pdo,array $viewer,string $publicId): ?array {
    $q=$pdo->prepare('SELECT ss.*,t.public_id team_public_id,t.name team_name,rp.public_id project_public_id,rp.title project_title FROM saved_searches ss LEFT JOIN teams t ON t.id=ss.team_id LEFT JOIN research_projects rp ON rp.id=ss.project_id WHERE ss.public_id=? LIMIT 1');$q->execute([$publicId]);$s=$q->fetch();if(!$s)return null;$uid=(int)$viewer['id'];if(($viewer['role']??'')==='admin'||(int)$s['owner_user_id']===$uid)return $s;
...
function search_saved_create(PDO $pdo,array $viewer,string $title,string $term,array $filters,string $visibility='private',?string $scopePublicId=null,bool $alerts=false): array {
    ...
    $public=ulid_like();$pdo->prepare('INSERT INTO saved_searches(public_id,owner_user_id,title,query_text,filters_json,visibility,team_id,project_id,alerts_enabled) VALUES(?,?,?,?,?,?,?,?,?)')->execute([$public,$viewer['id'],mb_substr($title,0,180),$term,json_encode(search_filters_from_input($filters),JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE),$visibility,$teamId,$projectId,$alerts?1:0]);return ['public_id'=>$public];
}
...
function search_saved_alert_toggle(PDO $pdo,array $viewer,string $publicId,bool $enabled): bool {
    $q=$pdo->prepare('UPDATE saved_searches SET alerts_enabled=? WHERE public_id=? AND owner_user_id=?');$q->execute([$enabled?1:0,$publicId,$viewer['id']]);return $q->rowCount()>0;
}
...
function search_remove_public_report_entities(PDO $pdo,string $reportPublicId): void {
    try{$pdo->prepare("DELETE FROM discovery_entity_mentions WHERE object_type='research_report' AND object_public_id=?")->execute([$reportPublicId]);}catch(PDOException $e){}
}
function search_sync_public_report_entities(PDO $pdo,string $reportPublicId): int {
    $q=$pdo->prepare("SELECT rr.public_id,rv.snapshot_json FROM research_reports rr JOIN research_report_versions rv ON rv.id=rr.current_version_id WHERE rr.public_id=? AND rr.status='published' AND rr.visibility='public' LIMIT 1");$q->execute([$reportPublicId]);$r=$q->fetch();if(!$r)return 0;$snapshot=json_decode((string)$r['snapshot_json'],true);if(!is_array($snapshot))return 0;$pdo->prepare("DELETE dem FROM discovery_entity_mentions dem WHERE dem.object_type='research_report' AND dem.object_public_id=?")->execute([$reportPublicId]);$count=0;
    foreach(($snapshot['entities']??[]) as $e){$name=trim((string)($e['name']??''));if($name==='')continue;$type=in_array((string)($e['type']??'other'),['person','company','organization','place','event','topic','date','other'],true)?(string)$e['type']:'other';$norm=search_normalized_name($name);$pdo->prepare("INSERT INTO discovery_entities(public_id,entity_type,canonical_name,normalized_name,description,status) VALUES(?,?,?,?,?,'active') ON DUPLICATE KEY UPDATE canonical_name=VALUES(canonical_name),description=COALESCE(NULLIF(VALUES(description),''),description),updated_at=NOW()")->execute([ulid_like(),$type,$name,$norm,mb_substr(trim((string)($e['description']??'')),0,4000)?:null]);$q=$pdo->prepare('SELECT id FROM discovery_entities WHERE entity_type=? AND normalized_name=?');$q->execute([$type,$norm]);$entityId=(int)$q->fetchColumn();if(!$entityId)continue;$pdo->prepare("INSERT INTO discovery_entity_mentions(entity_id,object_type,object_public_id,mention_weight,excerpt) VALUES(?,'research_report',?,3.00,?) ON DUPLICATE KEY UPDATE mention_weight=VALUES(mention_weight),excerpt=VALUES(excerpt),updated_at=NOW()")->execute([$entityId,$reportPublicId,mb_substr((string)($e['description']??''),0,1000)?:null]);$count++;
        foreach(($e['mentions']??[]) as $m){if(!empty($m['annotation_id'])&&annotation_access($pdo,(string)$m['annotation_id'],null)){$aq=$pdo->prepare('SELECT source_id,text_commentary FROM annotations WHERE public_id=?');$aq->execute([$m['annotation_id']]);$a=$aq->fetch();if($a)$pdo->prepare("INSERT INTO discovery_entity_mentions(entity_id,object_type,object_public_id,source_id,mention_weight,excerpt) VALUES(?,'annotation',?,?,2.00,?) ON DUPLICATE KEY UPDATE mention_weight=GREATEST(mention_weight,VALUES(mention_weight)),excerpt=VALUES(excerpt),updated_at=NOW()")->execute([$entityId,$m['annotation_id'],$a['source_id'],mb_substr((string)$a['text_commentary'],0,1000)?:null]);}if(!empty($m['source_id'])&&source_access($pdo,(string)$m['source_id'],null)){$sq=$pdo->prepare('SELECT id,title FROM sources WHERE public_id=?');$sq->execute([$m['source_id']]);$src=$sq->fetch();if($src)$pdo->prepare("INSERT INTO discovery_entity_mentions(entity_id,object_type,object_public_id,source_id,mention_weight,excerpt) VALUES(?,'source',?,?,1.50,?) ON DUPLICATE KEY UPDATE mention_weight=GREATEST(mention_weight,VALUES(mention_weight)),updated_at=NOW()")->execute([$entityId,$m['source_id'],$src['id'],mb_substr((string)$src['title'],0,1000)]);}}
    }return $count;
}
...
function search_related_sources(PDO $pdo,string $sourcePublicId,?array $viewer,int $limit=8): array {
    $source=source_access($pdo,$sourcePublicId,$viewer);if(!$source)return [];$q=$pdo->prepare("SELECT s2.public_id,s2.title,s2.domain,s2.canonical_url,COUNT(DISTINCT a2.user_id) shared_contributors FROM annotations a1 JOIN annotations a2 ON a2.user_id=a1.user_id AND a2.source_id<>a1.source_id JOIN sources s2 ON s2.id=a2.source_id WHERE a1.source_id=? AND a1.status='published' AND a2.status='published' AND COALESCE(s2.moderation_status,'visible')='visible' GROUP BY s2.id ORDER BY shared_contributors DESC,MAX(a2.published_at) DESC LIMIT 30");$q->execute([$source['id']]);$out=[];foreach($q->fetchAll() as $r){if(source_access($pdo,(string)$r['public_id'],$viewer))$out[]=$r;if(count($out)>=$limit)break;}return $out;
}
