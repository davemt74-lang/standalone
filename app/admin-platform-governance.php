...
function admin_platform_event(PDO $pdo,array $admin,string $eventType,string $subjectType,?string $subjectPublicId,mixed $before,mixed $after,string $reason,?string $correlation=null): void {
    ...
    $pdo->prepare("INSERT INTO admin_platform_events(public_id,actor_user_id,event_type,subject_type,subject_public_id,before_json,after_json,reason,correlation_id) VALUES(?,?,?,?,?,?,?,?,?)")->execute([ulid_like(),(int)$admin['id'],mb_substr($eventType,0,100),mb_substr($subjectType,0,80),$subjectPublicId!==null?mb_substr($subjectPublicId,0,255):null,$enc($before),$enc($after),admin_platform_reason($reason),$correlation!==null?mb_substr($correlation,0,96):null]);
    if(function_exists('admin_security_audit_record')&&admin_security_ready($pdo))admin_security_audit_record($pdo,$admin,['subject_type'=>$subjectType,'subject_public_id'=>$subjectPublicId,'event_type'=>$eventType,'source_domain'=>'platform','sensitivity'=>'restricted','before'=>$before,'after'=>$after,'metadata'=>$correlation?['correlation_id'=>$correlation]:null,'correlation_id'=>$correlation,'reason'=>$reason]);
}
...
function admin_platform_module_observed(PDO $pdo,array $module): array {
    ...
    return ['available'=>$available,'state'=>$available?'available':'unavailable','matches_desired'=>((string)$module['desired_state']==='enabled')===$available||in_array((string)$module['desired_state'],['maintenance','disabled'],true)];
}
...
function admin_platform_feature_action_preview(PDO $pdo,array $admin,string $featurePublicId,array $input): array {
    ...
    $status=in_array((string)($input['lifecycle_status']??$feature['lifecycle_status']),['draft','pilot','active','paused','retired'],true)?(string)$input['lifecycle_status']:(string)$feature['lifecycle_status'];$mode=in_array((string)($input['enforcement_mode']??$feature['enforcement_mode']),['observe','enforce'],true)?(string)$input['enforcement_mode']:(string)$feature['enforcement_mode'];$enabled=!empty($input['default_enabled']);$roll=max(0,min(100,(int)($input['rollout_percent']??$feature['rollout_percent'])));$packages=admin_platform_json_array($input['packages']??$feature['packages']);$accounts=admin_platform_json_array($input['accounts']??$feature['accounts']);$dependencies=admin_platform_json_array($input['dependencies']??$feature['dependencies']);if(in_array($feature['feature_key'],$dependencies,true))throw new InvalidArgumentException('A feature cannot depend on itself.');
