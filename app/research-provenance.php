...
function provenance_project_source_versions(PDO $pdo,int $projectId): array {
...
function provenance_project_annotations(PDO $pdo,int $projectId): array {
    ...
    foreach($q->fetchAll() as $r)$out[]=['id'=>$r['public_id'],'visibility'=>$r['visibility'],'status'=>$r['status'],'source_id'=>$r['source_public_id'],'source_version'=>(int)$r['source_version_number'],'capture_id'=>$r['capture_public_id'],'capture_type'=>$r['capture_type'],'selected_text_hash'=>$r['selected_text']!==null?hash('sha256',(string)$r['selected_text']):null,'commentary_hash'=>$r['text_commentary']!==null?hash('sha256',(string)$r['text_commentary']):null,'created_at'=>$r['created_at'],'updated_at'=>$r['updated_at']];
    return $out;
}

function provenance_project_claims(PDO $pdo,int $projectId): array {
...
function provenance_project_reports(PDO $pdo,array $viewer,int $projectId): array {
    $q=$pdo->prepare("SELECT rr.public_id report_public_id,rr.status report_status,rr.current_version_id,rv.id version_id,rv.public_id version_public_id,rv.version_number,rv.visibility,rv.title,rv.snapshot_json,rv.snapshot_hash,rv.created_at FROM research_reports rr JOIN research_report_versions rv ON rv.report_id=rr.id WHERE rr.project_id=? ORDER BY rv.version_number");$q->execute([$projectId]);$out=[];
    foreach($q->fetchAll() as $r){$computed=hash('sha256',(string)$r['snapshot_json']);$citations=[];if(function_exists('research_network_ready')&&research_network_ready($pdo)){$cq=$pdo->prepare("SELECT target_version_id,relation_type,note FROM research_report_version_citations WHERE source_version_id=? ORDER BY target_version_id");$cq->execute([$r['version_id']]);foreach($cq->fetchAll() as $edge){$target=research_network_version_meta_by_id($pdo,(int)$edge['target_version_id'],$viewer);$citations[]=$target?['available'=>true,'report_id'=>$target['report_public_id'],'version_id'=>$target['version_public_id'],'version_number'=>$target['version_number'],'relation'=>$edge['relation_type'],'note_hash'=>$edge['note']!==null?hash('sha256',(string)$edge['note']):null]:['available'=>false,'relation'=>$edge['relation_type']];}}
...
function provenance_project_manifest(PDO $pdo,array $viewer,string $projectPublic): array {
    ...
    $manifest=['schema'=>'annotated-provenance-v1','scope'=>['type'=>'project','project_id'=>$project['public_id']],'project'=>['public_id'=>$project['public_id'],'title'=>$project['title'],'description'=>$project['description'],'status'=>$project['status']??'active'],'source_versions'=>provenance_project_source_versions($pdo,(int)$project['id']),'annotations'=>provenance_project_annotations($pdo,(int)$project['id']),'claims'=>provenance_project_claims($pdo,(int)$project['id']),'findings'=>provenance_project_findings($pdo,(int)$project['id']),'report_versions'=>provenance_project_reports($pdo,$viewer,(int)$project['id']),'reviews'=>provenance_project_reviews($pdo,(int)$project['id']),'decision_memory'=>provenance_project_outcomes($pdo,$viewer,(int)$project['id'])];
...
function provenance_manifest_edges(array $manifest): array {
    ...
    foreach($manifest['claims']??[] as $c)foreach($c['evidence']??[] as $e){$add('source:'.$e['source_id'].':v'.$e['source_version'],'claim:'.$c['id'],'evidence',['relationship'=>$e['relationship']]);if(!empty($e['annotation_id']))$add('annotation:'.$e['annotation_id'],'claim:'.$c['id'],'annotation_evidence',['relationship'=>$e['relationship']]);}
...
function provenance_cognitive_observations(PDO $pdo,array $viewer,array &$items,int $limitProjects=12): void {
    ...
    foreach($q->fetchAll(PDO::FETCH_COLUMN) as $projectPublic){$ctx=provenance_project_context($pdo,$viewer,(string)$projectPublic);$invalid=(int)$ctx['integrity']['report_snapshots']['invalid'];if($invalid<=0)continue;cognitive_feed_add($items,['key'=>cognitive_feed_key('provenance_integrity','research_project',(string)$projectPublic,(string)$ctx['integrity']['manifest_hash']),'type'=>'provenance_integrity','section'=>'needs_attention','priority'=>'high','created_at'=>date('Y-m-d H:i:s'),'score_extra'=>25,'title'=>'Research provenance integrity mismatch','body'=>$invalid.' stored report snapshot hash'.($invalid===1?' does':'es do').' not match recomputed immutable snapshot content.','meta'=>['project'=>$projectPublic,'invalid_report_snapshots'=>$invalid],'actions'=>[cognitive_feed_action_link('Open provenance','/research-provenance.php?id='.rawurlencode((string)$projectPublic)),cognitive_feed_action_agent('Ask Agent','Explain the provenance integrity mismatch in this Research project. Do not alter any report or evidence.',[['type'=>'research','public_id'=>(string)$projectPublic]])]]);}}
}
