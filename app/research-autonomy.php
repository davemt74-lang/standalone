...
function research_autonomy_input_state(PDO $pdo,int $projectId): array {
    ...
    foreach([
      ...
      'stickies'=>"SELECT COUNT(*),COALESCE(MAX(rws.object_id),0),COALESCE(MAX(UNIX_TIMESTAMP(rws.updated_at)),0) FROM research_workspace_stickies rws JOIN research_workspace_objects rwo ON rwo.id=rws.object_id LEFT JOIN research_autonomy_artifacts raa ON raa.object_id=rwo.id WHERE rws.project_id=? AND rwo.status='active' AND raa.id IS NULL",
