...
function annotation_intelligence_apply_ai_output(PDO $pdo,string $annotationPublicId,string $output,string $runPublicId,int $modelId): array {
    ...
    $confidence=max(0,min(1,(float)($data['confidence']??0.65)));$hash=annotation_intelligence_input_hash($row);
    $pdo->prepare("INSERT INTO annotation_intelligence(annotation_id,status,input_hash,summary,topics_json,entities_json,claims_json,confidence,model_id,ai_run_public_id,prompt_version,last_error,processed_at)
      ...
      ->execute([$annotationId,$hash,$summary,json_encode($topics,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),json_encode($entities,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),json_encode($claims,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),$confidence,$modelId,$runPublicId]);
    $pdo->prepare("DELETE FROM annotation_relationships WHERE source_annotation_id=? AND generated_by='ai'")->execute([$annotationId]);
