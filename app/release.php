...
function release_worker_health(PDO $pdo): array {
    $expected=['media','transcription','source_monitor','ai','saved_search','research_automation','training','post_training'];$rows=[];try{$q=$pdo->query('SELECT * FROM worker_heartbeats');foreach($q->fetchAll() as $r)$rows[$r['worker_name']]=$r;}catch(PDOException $e){}
...
function release_queue_health(PDO $pdo): array {
    $tables=['media'=>'media_jobs','transcription'=>'transcription_jobs','source_monitor'=>'source_monitor_jobs','ai'=>'ai_jobs','research_automation'=>'research_automation_runs','training'=>'data_training_jobs','post_training'=>'data_post_training_plans'];$out=[];
