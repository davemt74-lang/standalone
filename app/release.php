...
function release_worker_health(PDO $pdo): array {
    $expected=['media','transcription','source_monitor','ai','saved_search','research_automation','training'];$rows=[];try{$q=$pdo->query('SELECT * FROM worker_heartbeats');foreach($q->fetchAll() as $r)$rows[$r['worker_name']]=$r;}catch(PDOException $e){}
