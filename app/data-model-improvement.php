...
function data_model_improvement_triage(PDO $pdo,array $viewer,string $publicId,array $input): array {
    data_model_improvement_require_admin($viewer);$case=data_model_improvement_case_get($pdo,$publicId);if(!$case)throw new RuntimeException('Improvement case not found.');
    ...
    data_model_improvement_event($pdo,(int)$case['id'],(int)$viewer['id'],'human_triage',['classification'=>$classification,'status'=>$status,'note'=>$note,'owner_user_id'=>$owner]);
...
function data_model_improvement_proposal_create(PDO $pdo,array $viewer,string $casePublicId,array $input): array {
    data_model_improvement_require_admin($viewer);$case=data_model_improvement_case_get($pdo,$casePublicId);if(!$case)throw new RuntimeException('Improvement case not found.');
    ...
    data_model_improvement_event($pdo,(int)$case['id'],(int)$viewer['id'],'proposal_created',['proposal_public_id'=>$public,'proposal_type'=>$type,'content_hash'=>$hash]);return data_model_improvement_proposal_get($pdo,$public)??[];
}
function data_model_improvement_proposal_update(PDO $pdo,array $viewer,string $publicId,array $input): array {
    data_model_improvement_require_admin($viewer);$p=data_model_improvement_proposal_get($pdo,$publicId);if(!$p)throw new RuntimeException('Improvement proposal not found.');if($p['status']!=='draft')throw new RuntimeException('Only draft improvement proposals can be edited.');
    ...
    $pdo->prepare('UPDATE data_model_improvement_proposals SET title=?,sanitized_input=?,sanitized_expected_output=?,sanitized_context=?,purpose_justification=?,redaction_attested=?,rights_attested=?,content_hash=?,updated_at=NOW() WHERE id=?')->execute([$m['title'],$m['sanitized_input'],$m['sanitized_expected_output'],$m['sanitized_context'],$m['purpose_justification'],$m['redaction_attested'],$m['rights_attested'],$hash,$p['id']]);
    data_model_improvement_event($pdo,(int)$p['case_id'],(int)$viewer['id'],'proposal_updated',['proposal_public_id'=>$publicId,'content_hash'=>$hash]);return data_model_improvement_proposal_get($pdo,$publicId)??[];
}
function data_model_improvement_proposal_approve(PDO $pdo,array $viewer,string $publicId): array {
    data_model_improvement_require_admin($viewer);$p=data_model_improvement_proposal_get($pdo,$publicId);if(!$p)throw new RuntimeException('Improvement proposal not found.');if($p['status']!=='draft')throw new RuntimeException('Only draft improvement proposals can be approved.');
    ...
    $pdo->prepare("UPDATE data_model_improvement_proposals SET status='approved',approval_hash=?,approved_by_user_id=?,approved_at=?,updated_at=NOW() WHERE id=?")->execute([$approvalHash,$viewer['id'],$approvedAt,$p['id']]);data_model_improvement_event($pdo,(int)$p['case_id'],(int)$viewer['id'],'proposal_approved',['proposal_public_id'=>$publicId,'approval_hash'=>$approvalHash]);return data_model_improvement_proposal_get($pdo,$publicId)??[];
}
function data_model_improvement_proposal_publish(PDO $pdo,array $viewer,string $publicId): array {
    data_model_improvement_require_admin($viewer);$p=data_model_improvement_proposal_get($pdo,$publicId);if(!$p)throw new RuntimeException('Improvement proposal not found.');if($p['status']!=='approved')throw new RuntimeException('Only an approved sanitized proposal can be published to the governed corpus.');
    ...
    $pdo->beginTransaction();try{
        $pdo->prepare("UPDATE data_model_improvement_proposals SET status='published',published_at=NOW(),updated_at=NOW() WHERE id=?")->execute([$p['id']]);
