...
function agent_chat_send(PDO $pdo,array $config,array $viewer,?string $conversationPublicId,string $prompt,array $contextItems=[],?string $clientMessageId=null): array {
    ...
    $retrievalContext=null;$researchAgent=null;
    ...
    $history=agent_chat_history_text($pdo,(int)$conversation['id'],16);$contextText=implode("\n\n",array_map(fn($x)=>$x['text'],$context));
    if($researchAgent&&empty($researchAgent['team_id'])&&function_exists('research_intelligence_delivery_agent_context')){try{$deliveryContext=research_intelligence_delivery_agent_context($pdo,$viewer,(string)$researchAgent['public_id']);if($deliveryContext!=='')$contextText.=($contextText!==''?"\n\n":'').$deliveryContext;}catch(Throwable $ignored){}}
    if($researchAgent&&function_exists('research_longitudinal_prompt_context')){try{$longitudinalContext=research_longitudinal_prompt_context($pdo,$viewer,(string)$researchAgent['public_id'],$prompt);if($longitudinalContext!=='')$contextText.=($contextText!==''?"\n\n":'').$longitudinalContext;}catch(Throwable $ignored){}}
