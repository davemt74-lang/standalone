...
function agent_chat_send(PDO $pdo,array $config,array $viewer,?string $conversationPublicId,string $prompt,array $contextItems=[],?string $clientMessageId=null): array {
    ...
    if($researchAgent&&function_exists('research_intelligence_delivery_agent_context')){try{$deliveryContext=research_intelligence_delivery_agent_context($pdo,$viewer,(string)$researchAgent['public_id']);if($deliveryContext!=='')$contextText.=($contextText!==''?"\n\n":'').$deliveryContext;}catch(Throwable $ignored){}}
