...
function agent_chat_send(PDO $pdo,array $config,array $viewer,?string $conversationPublicId,string $prompt,array $contextItems=[],?string $clientMessageId=null): array {
    ...
    if($researchAgent&&function_exists('research_longitudinal_agent_context')){try{$longitudinalContext=research_longitudinal_agent_context($pdo,$viewer,(string)$researchAgent['public_id'],30);if($longitudinalContext!=='')$contextText.=($contextText!==''?"\n\n":'').$longitudinalContext;}catch(Throwable $ignored){}}
