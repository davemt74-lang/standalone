...
function rate_limit_consume(PDO $pdo,string $scope,string $subject,int $limit,int $windowSeconds): array {
    ...
    try{
        ...
        if(($e->errorInfo[1]??null)===1146)return ['allowed'=>true,'hits'=>0,'limit'=>$limit,'retry_after'=>0,'degraded'=>true];
        throw $e;
    }
    return ['allowed'=>$hits<=$limit,'hits'=>$hits,'limit'=>$limit,'retry_after'=>max(1,$windowSeconds-($now%$windowSeconds)),'degraded'=>false];
}
