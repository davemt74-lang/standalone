...
function admin_ops_emit_alert(PDO $pdo,string $scanToken,array $alert): void {
    ...
    $sql="INSERT INTO admin_operations_alerts(public_id,fingerprint,account_id,severity,category,title,detail,source_type,source_public_id,target_url,status,last_scan_token)
      VALUES(?,?,?,?,?,?,?,?,?,?,'open',?)
      ON DUPLICATE KEY UPDATE account_id=VALUES(account_id),severity=VALUES(severity),category=VALUES(category),title=VALUES(title),detail=VALUES(detail),source_type=VALUES(source_type),source_public_id=VALUES(source_public_id),target_url=VALUES(target_url),last_seen_at=NOW(),last_scan_token=VALUES(last_scan_token),status=CASE WHEN status='resolved' OR (status='snoozed' AND snoozed_until IS NOT NULL AND snoozed_until<=NOW()) THEN 'open' ELSE status END,resolved_at=CASE WHEN status='resolved' THEN NULL ELSE resolved_at END,resolution_note=CASE WHEN status='resolved' THEN NULL ELSE resolution_note END";
...
function admin_ops_refresh_alerts(PDO $pdo): array {
    ...
    if(function_exists('commercial_billing_ready')&&commercial_billing_ready($pdo)){
        ...
        if(!empty($settings['automatic_tax_enabled'])){
            $q=$pdo->prepare("SELECT DISTINCT a.public_id account_public_id,a.name account_name FROM accounts a JOIN stripe_customers sc ON sc.account_id=a.id AND sc.mode=? LEFT JOIN account_billing_profiles bp ON bp.account_id=a.id WHERE a.status<>'closed' AND (bp.account_id IS NULL OR bp.address_country IS NULL OR bp.address_country='') ORDER BY a.name,a.id LIMIT 300");$q->execute([$mode]);
...
function admin_ops_alert_update(PDO $pdo,array $admin,string $publicId,string $operation,array $input=[]): array {
    ...
    elseif($operation==='snooze'){$hours=max(1,min(720,(int)($input['hours']??24)));$pdo->prepare("UPDATE admin_operations_alerts SET status='snoozed',snoozed_until=DATE_ADD(UTC_TIMESTAMP(),INTERVAL ? HOUR),resolution_note=?,resolved_at=NULL WHERE id=?")->execute([$hours,$reason,(int)$row['id']]);}
...
function admin_ops_action_preview(PDO $pdo,array $admin,string $actionType,string $accountPublicId,string $reason): array {
    admin_ops_require_admin($admin);if(!admin_ops_ready($pdo))throw new RuntimeException('Admin V2.0 operations requires migration 073.');$catalog=admin_ops_action_catalog();if(!isset($catalog[$actionType]))throw new InvalidArgumentException('Unsupported governed action.');$profile=admin_ops_operator_profile($pdo,$admin);if(!admin_ops_has_capability($profile,(string)$catalog[$actionType]['capability'])&&!admin_ops_has_capability($profile,'admin.*'))throw new RuntimeException('Operator capability is not authorized for this action.');$account=account_admin_get($pdo,$accountPublicId);if(!$account)throw new RuntimeException('Account not found.');$reason=admin_ops_reason($reason);
    ...
    $record=admin_ops_record_action($pdo,$admin,(int)$account['id'],$actionType,'previewed',(string)$catalog[$actionType]['risk'],$reason,['account_public_id'=>$account['public_id'],'preview'=>$preview],null);return ['record'=>$record,'preview'=>$preview];
}
...
function admin_ops_action_execute(PDO $pdo,array $config,array $admin,string $recordPublicId): array {
    admin_ops_require_admin($admin);$record=admin_ops_action_record($pdo,$recordPublicId);if(!$record)throw new RuntimeException('Governed action preview not found.');if((int)$record['actor_user_id']!==(int)$admin['id'])throw new RuntimeException('Only the administrator who created the preview may execute it.');if($record['status']!=='previewed')throw new RuntimeException('This governed action is no longer executable.');
    $catalog=admin_ops_action_catalog();$action=(string)$record['action_type'];if(!isset($catalog[$action]))throw new RuntimeException('Governed action is no longer supported.');$profile=admin_ops_operator_profile($pdo,$admin);if(!admin_ops_has_capability($profile,(string)$catalog[$action]['capability'])&&!admin_ops_has_capability($profile,'admin.*'))throw new RuntimeException('Operator capability is not authorized for this action.');
    $claim=$pdo->prepare("UPDATE admin_action_records SET status='executing' WHERE id=? AND status='previewed'");$claim->execute([(int)$record['id']]);if($claim->rowCount()!==1)throw new RuntimeException('Governed action was already claimed.');
    ...
    try{
        ...
        $pdo->prepare("UPDATE admin_action_records SET status='executed',result_json=?,executed_at=NOW() WHERE id=?")->execute([json_encode($safe,JSON_UNESCAPED_SLASHES|JSON_THROW_ON_ERROR),(int)$record['id']]);admin_ops_refresh_alerts($pdo);return admin_ops_action_record($pdo,$recordPublicId)??$record;
    }catch(Throwable $e){$pdo->prepare("UPDATE admin_action_records SET status='failed',result_json=?,executed_at=NOW() WHERE id=?")->execute([json_encode(['ok'=>false,'error'=>mb_substr($e->getMessage(),0,1000)],JSON_UNESCAPED_SLASHES),(int)$record['id']]);throw $e;}
}
...
function admin_ops_global_search(PDO $pdo,string $query,int $limit=40): array {
...
function admin_ops_account_timeline(PDO $pdo,int $accountId,int $limit=200): array {
...
function admin_ops_account_360(PDO $pdo,int $accountId): array {
    ...
    return ['account'=>$account,'stripe'=>$stripe,'billing_profile'=>$profile,'benefits'=>$benefits,'overage'=>$overage,'alerts'=>$alerts,'actions'=>$actions,'timeline'=>admin_ops_account_timeline($pdo,$accountId,200)];
}
...
function admin_ops_dashboard(PDO $pdo,array $admin,array $filters=[]): array {
    if(!admin_ops_ready($pdo))return ['ready'=>false,'alerts'=>[],'counts'=>[],'saved_views'=>[],'profile'=>admin_ops_operator_profile($pdo,$admin),'actions'=>[]];admin_ops_refresh_alerts($pdo);$filters['actor_user_id']=(int)$admin['id'];return ['ready'=>true,'alerts'=>admin_ops_alerts($pdo,$filters,200),'counts'=>admin_ops_alert_counts($pdo),'saved_views'=>admin_ops_saved_views($pdo,$admin),'profile'=>admin_ops_operator_profile($pdo,$admin),'actions'=>admin_ops_recent_actions($pdo,30),'admin_users'=>admin_ops_admin_users($pdo)];
}
function admin_ops_agent_context(PDO $pdo,array $viewer): string {
    if(($viewer['role']??'')!=='admin'||!admin_ops_ready($pdo))return '';$counts=admin_ops_alert_counts($pdo);$q=$pdo->query("SELECT COUNT(*) FROM admin_action_records WHERE status='failed' AND created_at>=DATE_SUB(UTC_TIMESTAMP(),INTERVAL 7 DAY)");$failed=(int)$q->fetchColumn();return "[ADMIN V2.0 OPERATIONS — READ ONLY]\nOpen operational alerts: ".(int)$counts['open']." (danger ".(int)$counts['danger'].", warning ".(int)$counts['warn']."). Failed governed Admin actions in the last 7 days: {$failed}. Account 360 combines existing authoritative ledgers; the Agent may explain and prioritize supplied operational context but may not resolve/snooze alerts, execute governed actions, change assignments, or mutate account/billing state.";
}
