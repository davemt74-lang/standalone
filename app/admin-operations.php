...
function admin_ops_action_preview(PDO $pdo,array $admin,string $actionType,string $accountPublicId,string $reason,array $params=[]): array {
    ...
    $account=account_admin_get($pdo,$accountPublicId);if(!$account)throw new RuntimeException('Account not found.');$reason=admin_ops_reason($reason);$params=is_array($params)?$params:[];
    ...
    elseif($actionType==='change_account_package'){
        ...
        $status=strtolower(trim((string)($params['status']??'')));if(!in_array($status,['active','suspended'],true))throw new InvalidArgumentException('Account lifecycle preview supports only active or suspended status.');if($status===(string)$account['status'])throw new RuntimeException('The account is already '.$status.'.');$params=['status'=>$status];$preview['current_status']=(string)$account['status'];$preview['target_status']=$status;
    }
