...
function admin_ops_account_timeline(PDO $pdo,int $accountId,int $limit=200): array {
    ...
    if(installer_table_exists($pdo,'admin_support_cases')&&installer_table_exists($pdo,'admin_support_case_events')){$q=$pdo->prepare("SELECT e.created_at,CONCAT('support_',e.event_type) event_type,e.body reason,u.username actor_username FROM admin_support_case_events e JOIN admin_support_cases c ON c.id=e.case_id LEFT JOIN users u ON u.id=e.actor_user_id WHERE c.account_id=? ORDER BY e.created_at DESC,e.id DESC LIMIT 100");$q->execute([$accountId]);$add('support',$q->fetchAll()?:[]);}
