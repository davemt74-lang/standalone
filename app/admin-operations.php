...
function admin_ops_action_catalog(): array {
    return [
        ...
        'change_account_package'=>['label'=>'Change account package','risk'=>'elevated','capability'=>'admin.accounts.manage','surface'=>'agent_account','description'=>'Change a manual-billing account to another active package through the authoritative Account 360 handler.'],
        'set_account_lifecycle'=>['label'=>'Suspend or reactivate account','risk'=>'elevated','capability'=>'admin.accounts.manage','surface'=>'agent_account','description'=>'Suspend or reactivate an account while preserving authoritative subscription-state rules.'],
...
function admin_ops_action_preview(PDO $pdo,array $admin,string $actionType,string $accountPublicId,string $reason,array $params=[]): array {
    ...
    $account=account_admin_get($pdo,$accountPublicId);if(!$account)throw new RuntimeException('Account not found.');$reason=admin_ops_reason($reason);$params=is_array($params)?$params:[];if(in_array($actionType,['change_account_package','set_account_lifecycle'],true)&&(!function_exists('admin_access_ready')||!admin_access_ready($pdo)))throw new RuntimeException('Admin V2.80 account changes require the V2.10 approval subsystem.');
    ...
    elseif($actionType==='change_account_package'){
        $packagePublic=trim((string)($params['package_public_id']??''));$package=$packagePublic!==''?subscription_package($pdo,$packagePublic):null;if(!$package||($package['status']??'')!=='active')throw new InvalidArgumentException('Choose a valid active target package.');if((int)$package['id']===(int)$account['package_id'])throw new RuntimeException('The account is already on that package.');if(($account['billing_source']??'manual')==='stripe'&&stripe_billing_ready($pdo)&&stripe_billing_current_subscription($pdo,(int)$account['id']))throw new RuntimeException('Stripe-managed accounts must change packages through Stripe billing.');
        $params=['package_public_id'=>(string)$package['public_id']];$preview['current_package']=(string)$account['package_name'];$preview['target_package']=(string)$package['name'];$preview['target_package_public_id']=(string)$package['public_id'];$preview['target_monthly_price']=(string)$package['monthly_price'];
    }elseif($actionType==='set_account_lifecycle'){
        $status=strtolower(trim((string)($params['status']??'')));if(!in_array($status,['active','suspended'],true))throw new InvalidArgumentException('Account lifecycle preview supports only active or suspended status.');$current=(string)$account['status'];if($status===$current)throw new RuntimeException('The account is already '.$status.'.');if(!in_array($current,['active','suspended'],true)||!(($current==='active'&&$status==='suspended')||($current==='suspended'&&$status==='active')))throw new RuntimeException('Admin Agent lifecycle changes are limited to active ↔ suspended transitions.');$params=['status'=>$status];$preview['current_status']=$current;$preview['target_status']=$status;
    }
    $record=admin_ops_record_action($pdo,$admin,(int)$account['id'],$actionType,'previewed',(string)$catalog[$actionType]['risk'],$reason,['account_public_id'=>$account['public_id'],'params'=>$params,'preview'=>$preview],null);
    if(function_exists('admin_access_bind_action_policy')&&admin_access_ready($pdo))$record=admin_access_bind_action_policy($pdo,$record);
    if(in_array($actionType,['change_account_package','set_account_lifecycle'],true)&&(int)($record['required_approvals']??0)<1&&function_exists('admin_access_ready')&&admin_access_ready($pdo)){
        $pdo->prepare("UPDATE admin_action_records SET required_approvals=1,approval_distinct_from_requester=1,approval_approver_capability='admin.actions.approve',risk_level='elevated' WHERE id=?")->execute([(int)$record['id']]);$record=admin_ops_action_record($pdo,(string)$record['public_id'])??$record;
    }
    $preview['risk_level']=(string)($record['risk_level']??$preview['risk_level']);$preview['required_approvals']=(int)($record['required_approvals']??0);return ['record'=>$record,'preview'=>$preview];
}
...
function admin_ops_action_execute(PDO $pdo,array $config,array $admin,string $recordPublicId): array {
    ...
    $request=json_decode((string)$record['request_json'],true)?:[];$accountPublic=(string)($request['account_public_id']??$record['account_public_id']??'');$params=is_array($request['params']??null)?$request['params']:[];
    try{
        ...
        elseif($action==='change_account_package')$result=account_admin_change_package($pdo,$admin,$accountPublic,(string)($params['package_public_id']??''),(string)$record['reason']);
        elseif($action==='set_account_lifecycle')$result=account_admin_update_lifecycle($pdo,$admin,$accountPublic,['status'=>(string)($params['status']??''),'reason'=>(string)$record['reason']]);
        ...
        $safe=['ok'=>true,'action_type'=>$action];if($accountPublic!=='')$safe['account_public_id']=$accountPublic;if(is_array($result)){foreach(['public_id','feature_key','subscription_status','status','package_public_id','package_name','enforcement_mode','rollout_percent','report'] as $key)if(array_key_exists($key,$result))$safe[$key]=$result[$key];}
