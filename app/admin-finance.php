...
function admin_finance_export_rows(PDO $pdo,string $type,array $filters=[]): array {
    ...
    if($type==='ledger'){
        $range=admin_finance_range((string)($filters['from']??''),(string)($filters['to']??''));$mode=admin_finance_mode($pdo);$q=$pdo->prepare("SELECT i.created_at,'invoice' record_type,a.public_id account_public_id,a.name account_name,i.stripe_invoice_id identifier,i.invoice_number label,COALESCE(i.total_cents,i.amount_due_cents,0) amount_cents,i.currency,i.status,i.amount_paid_cents,i.amount_remaining_cents,i.tax_amount_cents,i.discount_amount_cents FROM stripe_invoices i JOIN accounts a ON a.id=i.account_id WHERE i.mode=? AND i.created_at>=? AND i.created_at<? ORDER BY i.created_at DESC,i.id DESC");$q->execute([$mode,$range['from_at'],$range['to_exclusive']]);return $q->fetchAll()?:[];
    }
    throw new InvalidArgumentException('Unknown finance export type.');
}
function admin_finance_csv(array $rows): string {
    if(!$rows)return "no_records\n";$headers=array_keys($rows[0]);$f=fopen('php://temp','r+');fputcsv($f,$headers);foreach($rows as $row){$line=[];foreach($headers as $h){$v=$row[$h]??null;if(is_array($v)||is_object($v))$v=json_encode($v,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);$line[]=$v;}fputcsv($f,$line);}rewind($f);$csv=(string)stream_get_contents($f);fclose($f);return $csv;
}
