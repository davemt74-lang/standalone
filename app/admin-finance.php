...
function admin_finance_exception_update(PDO $pdo,array $admin,string $publicId,string $operation,array $input=[]): array {
    ...
    elseif($operation==='acknowledge'){if((string)$row['status']!=='open')throw new RuntimeException('Only open finance exceptions can be acknowledged.');$pdo->prepare("UPDATE admin_finance_reconciliation_exceptions SET status='acknowledged',assigned_user_id=COALESCE(assigned_user_id,?) WHERE id=?")->execute([(int)$admin['id'],(int)$row['id']]);}
    elseif($operation==='resolve'){if(!in_array((string)$row['status'],['open','acknowledged'],true))throw new RuntimeException('Only open or acknowledged finance exceptions can be resolved.');$pdo->prepare("UPDATE admin_finance_reconciliation_exceptions SET status='resolved',resolved_at=NOW(),resolution_reason=?,assigned_user_id=COALESCE(assigned_user_id,?) WHERE id=?")->execute([$reason,(int)$admin['id'],(int)$row['id']]);}
    elseif($operation==='ignore'){if(!in_array((string)$row['status'],['open','acknowledged'],true))throw new RuntimeException('Only open or acknowledged finance exceptions can be ignored.');$pdo->prepare("UPDATE admin_finance_reconciliation_exceptions SET status='ignored',resolved_at=NOW(),resolution_reason=?,assigned_user_id=COALESCE(assigned_user_id,?) WHERE id=?")->execute([$reason,(int)$admin['id'],(int)$row['id']]);}
    elseif($operation==='reopen'){if(!in_array((string)$row['status'],['resolved','ignored'],true))throw new RuntimeException('Only resolved or ignored finance exceptions can be reopened.');$pdo->prepare("UPDATE admin_finance_reconciliation_exceptions SET status='open',resolved_at=NULL,resolution_reason=NULL WHERE id=?")->execute([(int)$row['id']]);}
...
function admin_finance_period_close(PDO $pdo,array $admin,string $from,string $to,string $reason): array {
    ...
    $critical=$pdo->prepare("SELECT COUNT(*) FROM admin_finance_reconciliation_exceptions WHERE stripe_mode=? AND severity='critical' AND status IN ('open','acknowledged')");$critical->execute([$mode]);if((int)$critical->fetchColumn()>0)throw new RuntimeException('Resolve or explicitly ignore critical finance reconciliation exceptions before closing a reporting period.');
