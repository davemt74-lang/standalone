...
function data_model_observability_signal_for_run(PDO $pdo,string $runPublicId,string $signalType,float $value,string $sourceType='system',?string $sourcePublicId=null,?int $actorUserId=null,?string $note=null): ?array {
    ...
    $q=$pdo->prepare('SELECT * FROM data_model_outcome_signals WHERE observation_id=? AND signal_type=? AND source_type=? AND source_public_id=? LIMIT 1');$q->execute([$row['observation_id'],$signalType,$sourceType,$sourcePublicId]);return $q->fetch()?:null;
}
function data_model_observability_policy_material(array $p): array {
    return ['deployment_id'=>(int)$p['deployment_id'],'enabled'=>(int)$p['enabled'],'window_minutes'=>(int)$p['window_minutes'],'min_samples'=>(int)$p['min_samples'],'max_failure_rate'=>(float)$p['max_failure_rate'],'max_p95_latency_ms'=>(int)$p['max_p95_latency_ms'],'max_avg_input_tokens'=>$p['max_avg_input_tokens']!==null?(int)$p['max_avg_input_tokens']:null,'max_avg_output_tokens'=>$p['max_avg_output_tokens']!==null?(int)$p['max_avg_output_tokens']:null,'max_avg_cost_micros'=>$p['max_avg_cost_micros']!==null?(int)$p['max_avg_cost_micros']:null,'min_avg_outcome_score'=>$p['min_avg_outcome_score']!==null?(float)$p['min_avg_outcome_score']:null];
}
...
function data_model_observability_policy_get(PDO $pdo,int $deploymentId,bool $create=true): ?array {
    ...
    $p=['deployment_id'=>$deploymentId,'enabled'=>1,'window_minutes'=>max(15,min(10080,(int)($monitor['monitoring_window_minutes']??120))),'min_samples'=>5,'max_failure_rate'=>0.10,'max_p95_latency_ms'=>30000,'max_avg_input_tokens'=>null,'max_avg_output_tokens'=>null,'max_avg_cost_micros'=>null,'min_avg_outcome_score'=>null];$hash=data_model_observability_policy_hash($p);
    $pdo->prepare('INSERT IGNORE INTO data_model_monitoring_policies(deployment_id,enabled,window_minutes,min_samples,max_failure_rate,max_p95_latency_ms,max_avg_input_tokens,max_avg_output_tokens,max_avg_cost_micros,min_avg_outcome_score,policy_hash) VALUES(?,?,?,?,?,?,?,?,?,?,?)')
      ->execute([$deploymentId,1,$p['window_minutes'],$p['min_samples'],$p['max_failure_rate'],$p['max_p95_latency_ms'],null,null,null,null,$hash]);
...
function data_model_observability_policy_update(PDO $pdo,array $viewer,int $deploymentId,array $input): array {
    ...
    $p=['deployment_id'=>$deploymentId,'enabled'=>isset($input['enabled'])?1:0,'window_minutes'=>max(15,min(10080,(int)($input['window_minutes']??120))),'min_samples'=>max(1,min(10000,(int)($input['min_samples']??5))),'max_failure_rate'=>max(0,min(1,(float)($input['max_failure_rate']??0.1))),'max_p95_latency_ms'=>max(1,min(3600000,(int)($input['max_p95_latency_ms']??30000))),'max_avg_input_tokens'=>$nullableInt('max_avg_input_tokens'),'max_avg_output_tokens'=>$nullableInt('max_avg_output_tokens'),'max_avg_cost_micros'=>$nullableInt('max_avg_cost_micros'),'min_avg_outcome_score'=>$nullableFloat('min_avg_outcome_score')];if($p['min_avg_outcome_score']!==null)$p['min_avg_outcome_score']=max(-1,min(1,$p['min_avg_outcome_score']));$hash=data_model_observability_policy_hash($p);
    $pdo->prepare('UPDATE data_model_monitoring_policies SET enabled=?,window_minutes=?,min_samples=?,max_failure_rate=?,max_p95_latency_ms=?,max_avg_input_tokens=?,max_avg_output_tokens=?,max_avg_cost_micros=?,min_avg_outcome_score=?,policy_hash=?,updated_by_user_id=? WHERE deployment_id=?')
      ->execute([$p['enabled'],$p['window_minutes'],$p['min_samples'],$p['max_failure_rate'],$p['max_p95_latency_ms'],$p['max_avg_input_tokens'],$p['max_avg_output_tokens'],$p['max_avg_cost_micros'],$p['min_avg_outcome_score'],$hash,$viewer['id'],$deploymentId]);
...
function data_model_observability_incident_upsert(PDO $pdo,array $snapshot,array $breach): array {
    ...
    $id=(int)$pdo->lastInsertId();data_model_observability_incident_event($pdo,$id,null,'opened',null,'open',$breach['summary'],$evidence);$q=$pdo->prepare('SELECT * FROM data_model_incidents WHERE id=?');$q->execute([$id]);$incident=$q->fetch();data_model_observability_notify_incident($pdo,$incident);return $incident;
}
function data_model_observability_evaluate_deployment(PDO $pdo,int $deploymentId,bool $force=false,?string $onlyRoute=null): array {
    ...
    foreach($routes as $route){
        ...
        $q=$pdo->prepare('SELECT * FROM data_model_observations WHERE deployment_id=? AND route_key=? AND observed_at>=? ORDER BY observed_at ASC,id ASC');$q->execute([$deploymentId,$route,$windowStart]);$rows=$q->fetchAll();$count=count($rows);$failures=0;$lat=[];$ins=[];$outs=[];$costs=[];$shadow=0;$served=0;
        foreach($rows as $r){if($r['run_status']!=='completed')$failures++;if($r['latency_ms']!==null)$lat[]=(float)$r['latency_ms'];if($r['input_tokens']!==null)$ins[]=(float)$r['input_tokens'];if($r['output_tokens']!==null)$outs[]=(float)$r['output_tokens'];if($r['estimated_cost_micros']!==null)$costs[]=(float)$r['estimated_cost_micros'];if((int)$r['is_shadow'])$shadow++;else$served++;}
        $q=$pdo->prepare('SELECT s.signal_value FROM data_model_outcome_signals s JOIN data_model_observations o ON o.id=s.observation_id WHERE o.deployment_id=? AND o.route_key=? AND o.observed_at>=?');$q->execute([$deploymentId,$route,$windowStart]);$signals=array_map('floatval',$q->fetchAll(PDO::FETCH_COLUMN));
        $avg=fn(array $v)=>$v?array_sum($v)/count($v):null;$failureRate=$count?$failures/$count:0.0;$metrics=[
          'sample_count'=>$count,'completed_count'=>$count-$failures,'failure_count'=>$failures,'failure_rate'=>$failureRate,
          'avg_latency_ms'=>$avg($lat),'p95_latency_ms'=>data_model_observability_percentile($lat,0.95),
          'avg_input_tokens'=>$avg($ins),'avg_output_tokens'=>$avg($outs),'avg_cost_micros'=>$avg($costs),'total_cost_micros'=>$costs?array_sum($costs):null,
          'outcome_signal_count'=>count($signals),'avg_outcome_score'=>$avg($signals),'shadow_candidate_count'=>$shadow,'served_baseline_or_candidate_count'=>$served,
        ];
        $breaches=[];$enough=$count>=(int)$policy['min_samples'];
        if($enough&&$failureRate>(float)$policy['max_failure_rate'])$breaches[]=['metric'=>'failure_rate','current'=>$failureRate,'threshold'=>(float)$policy['max_failure_rate'],'severity'=>'critical','summary'=>'Observed failure rate exceeds the configured production threshold.'];
        if($enough&&$metrics['p95_latency_ms']!==null&&$metrics['p95_latency_ms']>(float)$policy['max_p95_latency_ms'])$breaches[]=['metric'=>'p95_latency_ms','current'=>$metrics['p95_latency_ms'],'threshold'=>(float)$policy['max_p95_latency_ms'],'severity'=>'critical','summary'=>'Observed p95 latency exceeds the configured production threshold.'];
        ...
        if($enough&&$policy['min_avg_outcome_score']!==null&&$metrics['avg_outcome_score']!==null&&$metrics['avg_outcome_score']<(float)$policy['min_avg_outcome_score'])$breaches[]=['metric'=>'avg_outcome_score','current'=>$metrics['avg_outcome_score'],'threshold'=>(float)$policy['min_avg_outcome_score'],'severity'=>'warning','summary'=>'Observed outcome score is below the configured production threshold.'];
