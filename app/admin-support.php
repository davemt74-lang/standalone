...
function admin_support_create(PDO $pdo,array $admin,array $input): array {
    ...
    $status=$assigned?'assigned':'open';$pdo->prepare("INSERT INTO admin_support_cases(public_id,account_id,user_id,created_by_user_id,assigned_user_id,title,category,priority,status,source,sla_due_at,last_operator_activity_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,NOW())")->execute([ulid_like(),$account?(int)$account['id']:null,$user?(int)$user['id']:null,(int)$admin['id'],$assigned,$title,$category,$priority,$status,$source,admin_support_sla_due($priority)]);
    $id=(int)$pdo->lastInsertId();$case=admin_support_case($pdo,$id)??throw new RuntimeException('Support case could not be reloaded.');admin_support_event($pdo,$id,(int)$admin['id'],'case_created',$description,'internal',['priority'=>$priority,'category'=>$category,'duplicate_candidates'=>array_column($duplicates,'public_id')]);
...
function admin_support_update(PDO $pdo,array $admin,string $casePublicId,string $operation,array $input=[]): array {
    ...
    elseif($operation==='status'){$status=in_array((string)($input['status']??''),['open','assigned','waiting_customer','waiting_internal','escalated','resolved','closed'],true)?(string)$input['status']:throw new InvalidArgumentException('Invalid case status.');$sql="UPDATE admin_support_cases SET status=?,last_operator_activity_at=NOW(),first_response_at=COALESCE(first_response_at,NOW()),resolved_at=CASE WHEN ?='resolved' THEN NOW() WHEN status='resolved' AND ?<>'resolved' THEN NULL ELSE resolved_at END,closed_at=CASE WHEN ?='closed' THEN NOW() ELSE closed_at END WHERE id=?";$pdo->prepare($sql)->execute([$status,$status,$status,$status,$id]);admin_support_event($pdo,$id,(int)$admin['id'],$status==='resolved'?'case_resolved':($status==='closed'?'case_closed':'status_changed'),$reason,'internal',['status'=>$status]);}
