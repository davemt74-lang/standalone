...
function admin_support_create(PDO $pdo,array $admin,array $input): array {
    ...
    $priorityRaw=(string)($input['priority']??'normal');$priority=in_array($priorityRaw,['low','normal','high','urgent'],true)?$priorityRaw:'normal';$sourceRaw=(string)($input['source']??'admin');$source=in_array($sourceRaw,['admin','customer','system','alert'],true)?$sourceRaw:'admin';$description=admin_support_reason((string)($input['description']??''),'Support case created.');
...
function admin_support_escalation_capability(string $team): string {
    return match($team){'billing'=>'admin.billing.view','operations'=>'admin.operations.manage','ai_models'=>'admin.models.view','research_data'=>'admin.research_data.view','trust'=>'admin.trust.view','security'=>'admin.roles.view',default=>'admin.support.view'};
}
function admin_support_filters(array $input): array {
    $out=[];foreach(['status','priority','category','assigned','team','q'] as $key){$v=trim((string)($input[$key]??''));if($v!=='')$out[$key]=$v;}if(!empty($input['sla_risk']))$out['sla_risk']='1';return $out;
}
function admin_support_cases(PDO $pdo,array $viewer,array $filters=[],int $limit=250): array {
    ...
    if(isset($filters['team'])){$where[]='c.escalation_team=?';$params[]=$filters['team'];}
...
function admin_support_update(PDO $pdo,array $admin,string $casePublicId,string $operation,array $input=[]): array {
    ...
    elseif($operation==='status'){$status=in_array((string)($input['status']??''),['open','assigned','waiting_customer','waiting_internal','escalated'],true)?(string)$input['status']:throw new InvalidArgumentException('Invalid case status.');$was=(string)$case['status'];$sql="UPDATE admin_support_cases SET status=?,last_operator_activity_at=NOW(),resolved_at=CASE WHEN ?='resolved' THEN NOW() WHEN ?='resolved' AND ?<>'resolved' THEN NULL ELSE resolved_at END,closed_at=CASE WHEN ?='closed' THEN NOW() WHEN ?='closed' AND ?<>'closed' THEN NULL ELSE closed_at END WHERE id=?";$pdo->prepare($sql)->execute([$status,$status,$was,$status,$status,$was,$status,$id]);admin_support_event($pdo,$id,(int)$admin['id'],$status==='resolved'?'case_resolved':($status==='closed'?'case_closed':'status_changed'),$reason,'internal',['status'=>$status]);}
    elseif($operation==='escalate'){$team=preg_replace('/[^a-z0-9_\-]+/','_',strtolower(trim((string)($input['team']??''))))?:'';if(!in_array($team,['billing','operations','ai_models','research_data','trust','security'],true))throw new InvalidArgumentException('Escalation team is invalid.');$pdo->prepare("UPDATE admin_support_cases SET status='escalated',escalation_team=?,escalation_reason=?,last_operator_activity_at=NOW() WHERE id=?")->execute([$team,mb_substr($reason,0,500),$id]);admin_support_event($pdo,$id,(int)$admin['id'],'case_escalated',$reason,'internal',['team'=>$team]);if(function_exists('notification_create')){$cap=admin_support_escalation_capability($team);foreach(admin_support_admin_candidates($pdo,'admin.support.view') as $target)if(admin_access_has_capability($pdo,$target,$cap))notification_create($pdo,(int)$target['id'],(int)$admin['id'],'support_escalated','support_case',$casePublicId,'Support case escalated to '.ucwords(str_replace('_',' ',$team)).': '.(string)$case['title'],['dedupe_key'=>'support-escalated-'.$casePublicId.'-'.$team.'-'.$target['id'].'-'.hash('sha256',$reason),'group_key'=>'support-case-'.$casePublicId]);}}
    ...
    elseif($operation==='reopen'){if(!in_array((string)$case['status'],['resolved','closed'],true))throw new RuntimeException('Only resolved or closed cases can be reopened.');$pdo->prepare("UPDATE admin_support_cases SET status='open',resolved_at=NULL,closed_at=NULL,resolution_summary=NULL,last_operator_activity_at=NOW(),sla_due_at=? WHERE id=?")->execute([admin_support_sla_due((string)$case['priority']),$id]);admin_support_event($pdo,$id,(int)$admin['id'],'case_reopened',$reason,'internal');}
...
function admin_support_case_timeline(PDO $pdo,array $case,int $limit=300,?array $viewer=null): array {
    ...
    if(!empty($case['account_id'])&&function_exists('admin_ops_account_timeline'))foreach(admin_ops_account_timeline($pdo,(int)$case['account_id'],$limit,null) as $e){$e['visibility']='internal';$rows[]=$e;}
