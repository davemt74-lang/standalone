...
function profile_network_notify_followers_report(PDO $pdo,array $publisher,array $publish): int {
    ...
    foreach($q->fetchAll(PDO::FETCH_COLUMN) as $uid){$uid=(int)$uid;if(notification_create($pdo,$uid,(int)$publisher['id'],'research_followed_publication','research_report',$report,$publisher['display_name'].' published Research: '.$r['title'],['category'=>'research','dedupe_key'=>'followed-research:'.$report.':v'.(int)($publish['version_number']??1).':'.$uid,'group_key'=>'followed-research:'.$report,'context'=>['version_number'=>(int)($publish['version_number']??1),'actor_public_id'=>$publisher['public_id']]]))$sent++;}
    return $sent;
}
...
function profile_network_add_to_agent(PDO $pdo,array $config,array $viewer,string $agentPublic,string $type,string $publicId): array {
    ...
    $source=ensure_source($pdo,$url,(string)$r['title']);$snapshot=json_decode((string)$r['snapshot_json'],true)?:[];$text=profile_network_report_plaintext($snapshot,(string)($r['summary']??''));$hash=hash('sha256',$text);$snapshotHash=(string)$r['snapshot_hash'];
