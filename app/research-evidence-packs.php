...
function research_evidence_pack_cognitive_observations(PDO $pdo,array $viewer,array &$items,int $limitProjects=10): void {
    ...
    foreach($q->fetchAll(PDO::FETCH_COLUMN) as $projectPublic){$packs=research_evidence_pack_list($pdo,$viewer,(string)$projectPublic,12);$drift=array_values(array_filter($packs,fn($p)=>!$p['current_matches_pack']));if(!$drift)continue;$latest=$drift[0];cognitive_feed_add($items,[
