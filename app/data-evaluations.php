...
function data_evaluation_result_material(array $row): array {
    $retrieved=is_array($row['retrieved_json']??null)?$row['retrieved_json']:json_decode((string)($row['retrieved_json']??''),true);if(!is_array($retrieved))$retrieved=[];
    $citations=is_array($row['citations_json']??null)?$row['citations_json']:json_decode((string)($row['citations_json']??''),true);if(!is_array($citations))$citations=[];
    $metrics=is_array($row['metrics_json']??null)?$row['metrics_json']:json_decode((string)($row['metrics_json']??''),true);if(!is_array($metrics))$metrics=[];
    $response=(string)($row['response_text']??'');$responseHash=$response!==''?hash('sha256',$response):null;$metricsJson=data_attribution_encode($metrics);$metricsHash=hash('sha256',$metricsJson);
    $core=['case_id'=>(int)$row['case_id'],'rank_of_expected'=>$row['rank_of_expected']!==null?(int)$row['rank_of_expected']:null,'retrieved'=>$retrieved,'response_hash'=>$responseHash,'citations'=>$citations,'metrics_hash'=>$metricsHash,'passed'=>(int)$row['passed']];
    return ['retrieved'=>$retrieved,'citations'=>$citations,'metrics'=>$metrics,'response_hash'=>$responseHash,'metrics_hash'=>$metricsHash,'result_hash'=>data_attribution_hash($core)];
}
function data_evaluation_result_insert(PDO $pdo,int $runId,array $case,array $ranked,?string $response,array $metrics,bool $passed,array $citations=[]): void {
    $expectedRank=null;foreach($ranked['all'] as $r)if((int)$r['dataset_item_id']===(int)$case['expected_dataset_item_id']){$expectedRank=(int)$r['rank'];break;}$retrievedJson=data_attribution_encode($ranked['top']);$citationsJson=$citations?data_attribution_encode($citations):null;$metricsJson=data_attribution_encode($metrics);$metricsHash=hash('sha256',$metricsJson);$responseHash=$response!==null?hash('sha256',$response):null;
    $material=data_evaluation_result_material(['case_id'=>$case['id'],'rank_of_expected'=>$expectedRank,'retrieved_json'=>$retrievedJson,'response_text'=>$response,'citations_json'=>$citationsJson,'metrics_json'=>$metricsJson,'passed'=>$passed?1:0]);
    $pdo->prepare('INSERT INTO data_evaluation_results(run_id,case_id,rank_of_expected,retrieved_json,response_text,response_hash,citations_json,metrics_json,metrics_hash,result_hash,passed) VALUES(?,?,?,?,?,?,?,?,?,?,?)')
        ->execute([$runId,$case['id'],$expectedRank,$retrievedJson,$response,$responseHash,$citationsJson,$metricsJson,$metricsHash,$material['result_hash'],$passed?1:0]);
}
...
function data_evaluation_execute_run(PDO $pdo,array $config,int $runId): array {
    ...
    try{
        ...
        foreach($cases as $case){
            $caseStarted=microtime(true);$ranked=data_evaluation_rank_rows($rows,(string)$case['query_text'],$topK);$expectedRank=null;$expectedScore=0.0;foreach($ranked['all'] as $hit)if((int)$hit['dataset_item_id']===(int)$case['expected_dataset_item_id']){$expectedRank=(int)$hit['rank'];$expectedScore=(float)$hit['score'];break;}
            $hasEvidence=$expectedRank!==null&&$expectedScore>0.0;$base=['expected_rank'=>$expectedRank??0,'expected_score'=>$expectedScore,'hit_at_1'=>$hasEvidence&&$expectedRank===1?1:0,'hit_at_3'=>$hasEvidence&&$expectedRank<=3?1:0,'hit_at_5'=>$hasEvidence&&$expectedRank<=5?1:0,'mrr'=>$hasEvidence?1/$expectedRank:0.0,'top_score'=>(float)($ranked['top'][0]['score']??0)];
            if($run['benchmark_type']==='retrieval'){
                $base['latency_ms']=(microtime(true)-$caseStarted)*1000;data_evaluation_result_insert($pdo,$runId,$case,$ranked,null,$base,$hasEvidence&&$expectedRank<=$topK);continue;
            }
            $mp=data_evaluation_model_prompt($suite,$case,$ranked,$rowsById);$model=ai_generate($pdo,$config,(int)$run['model_id'],$mp['system'],$mp['prompt'],2048);$response=(string)$model['text'];$citations=data_evaluation_parse_citations($response);$expectedCorpus=(string)$case['expected_corpus_public_id'];$citationHit=in_array($expectedCorpus,$citations,true)?1:0;$ground=data_evaluation_overlap($response,$mp['context_text']);$reference=trim((string)($case['reference_answer']??''));$ref=$reference!==''?data_evaluation_overlap($reference,$response):['f1'=>0.0,'coverage'=>0.0];$metrics=array_merge($base,['expected_citation'=>$citationHit,'grounded_token_ratio'=>$ground['coverage'],'latency_ms'=>(microtime(true)-$caseStarted)*1000]);if($reference!=='')$metrics['reference_token_f1']=(float)$ref['f1'];if($model['input_tokens']!==null)$metrics['input_tokens']=(int)$model['input_tokens'];if($model['output_tokens']!==null)$metrics['output_tokens']=(int)$model['output_tokens'];$pass=$citationHit===1&&$ground['coverage']>=0.35&&($reference===''||$ref['f1']>=0.20);
            data_evaluation_result_insert($pdo,$runId,$case,$ranked,$response,$metrics,$pass,$citations);
        }
        $summary=data_evaluation_summary($pdo,$runId,(string)$run['benchmark_type']);$summaryJson=data_attribution_encode($summary);$summaryHash=hash('sha256',$summaryJson);$q=$pdo->prepare('SELECT result_hash FROM data_evaluation_results WHERE run_id=? ORDER BY case_id');$q->execute([$runId]);$resultHashes=$q->fetchAll(PDO::FETCH_COLUMN);$runHash=data_attribution_hash(['run_public_id'=>$run['public_id'],'dataset_manifest_hash'=>$run['dataset_manifest_hash'],'suite_config_hash'=>$run['suite_config_hash'],'cases_hash'=>$run['cases_hash'],'model_snapshot'=>json_decode((string)($run['model_snapshot_json']??''),true),'result_hashes'=>$resultHashes,'summary_hash'=>$summaryHash]);$pdo->prepare("UPDATE data_evaluation_runs SET status='completed',result_summary_json=?,result_summary_hash=?,run_hash=?,completed_at=NOW() WHERE id=?")->execute([$summaryJson,$summaryHash,$runHash,$runId]);data_evaluation_event($pdo,(int)$run['suite_id'],$runId,null,'run_completed',['summary_hash'=>$summaryHash,'run_hash'=>$runHash,'case_count'=>$summary['case_count']]);return data_evaluation_run_get($pdo,(string)$run['public_id'])??[];
...
function data_evaluation_run_integrity(PDO $pdo,array $run): array {
    if($run['status']!=='completed')return ['ok'=>false,'reason'=>'run_not_completed','invalid_results'=>0,'summary_ok'=>false,'run_hash_ok'=>false];
    $q=$pdo->prepare('SELECT * FROM data_evaluation_results WHERE run_id=? ORDER BY case_id');$q->execute([$run['id']]);$rows=$q->fetchAll();$invalid=0;$hashes=[];
    foreach($rows as $row){$m=data_evaluation_result_material($row);$hashes[]=$m['result_hash'];if(!hash_equals((string)$row['metrics_hash'],$m['metrics_hash'])||!hash_equals((string)($row['response_hash']??''),(string)($m['response_hash']??''))||!hash_equals((string)$row['result_hash'],$m['result_hash']))$invalid++;}
    $summary=json_decode((string)($run['result_summary_json']??''),true);if(!is_array($summary))$summary=[];$summaryHash=hash('sha256',data_attribution_encode($summary));$summaryOk=hash_equals((string)($run['result_summary_hash']??''),$summaryHash);
    $runHash=data_attribution_hash(['run_public_id'=>$run['public_id'],'dataset_manifest_hash'=>$run['dataset_manifest_hash'],'suite_config_hash'=>$run['suite_config_hash'],'cases_hash'=>$run['cases_hash'],'model_snapshot'=>json_decode((string)($run['model_snapshot_json']??''),true),'result_hashes'=>$hashes,'summary_hash'=>$summaryHash]);$runHashOk=hash_equals((string)($run['run_hash']??''),$runHash);
    return ['ok'=>$invalid===0&&$summaryOk&&$runHashOk,'reason'=>$invalid>0?'result_integrity_failure':(!$summaryOk?'summary_integrity_failure':(!$runHashOk?'run_integrity_failure':'current')),'invalid_results'=>$invalid,'summary_ok'=>$summaryOk,'run_hash_ok'=>$runHashOk,'computed_run_hash'=>$runHash,'stored_run_hash'=>$run['run_hash']];
}
function data_evaluation_run_requeue(PDO $pdo,array $viewer,string $runPublicId): array {
    data_evaluation_require_admin($viewer);$run=data_evaluation_run_get($pdo,$runPublicId);if(!$run)throw new RuntimeException('Evaluation run not found.');$status=(string)$run['status'];$stale=$status==='processing'&&!empty($run['started_at'])&&strtotime((string)$run['started_at'])<time()-900;if($status!=='failed'&&!$stale)throw new RuntimeException('Only failed or stale processing runs can be requeued.');
    $pdo->prepare("UPDATE data_evaluation_runs SET status='queued',error_text=NULL,started_at=NULL,completed_at=NULL,result_summary_json=NULL,result_summary_hash=NULL,run_hash=NULL WHERE id=?")->execute([$run['id']]);$pdo->prepare('DELETE FROM data_evaluation_results WHERE run_id=?')->execute([$run['id']]);data_evaluation_event($pdo,(int)$run['suite_id'],(int)$run['id'],(int)$viewer['id'],'run_requeued',['previous_status'=>$status]);return data_evaluation_run_get($pdo,$runPublicId)??[];
}
...
function data_evaluation_set_baseline(PDO $pdo,array $viewer,string $suitePublicId,string $runPublicId): array {
    data_evaluation_require_admin($viewer);$s=data_evaluation_suite_get($pdo,$suitePublicId);$r=data_evaluation_run_get($pdo,$runPublicId);if(!$s||!$r||(int)$r['suite_id']!==(int)$s['id'])throw new RuntimeException('Run does not belong to this suite.');if($r['status']!=='completed')throw new RuntimeException('Only completed runs can become baselines.');$integrity=data_evaluation_run_integrity($pdo,$r);if(!$integrity['ok'])throw new RuntimeException('A run with failed integrity cannot become a regression baseline.');
...
function data_evaluation_regression(PDO $pdo,array $suite,array $run): ?array {
    $baselineId=(int)($suite['baseline_run_id']??0);if(!$baselineId||$run['status']!=='completed'||(int)$run['id']===$baselineId)return null;$currentIntegrity=data_evaluation_run_integrity($pdo,$run);if(!$currentIntegrity['ok'])return ['available'=>false,'reason'=>'current_run_integrity_failure','regressed_metrics'=>0,'deltas'=>[]];$q=$pdo->prepare("SELECT * FROM data_evaluation_runs WHERE id=? AND status='completed'");$q->execute([$baselineId]);$b=$q->fetch();if(!$b)return null;$baselineIntegrity=data_evaluation_run_integrity($pdo,$b);if(!$baselineIntegrity['ok'])return ['available'=>false,'reason'=>'baseline_integrity_failure','regressed_metrics'=>0,'deltas'=>[]];$base=json_decode((string)$b['result_summary_json'],true)?:[];$cur=$run['summary']??json_decode((string)$run['result_summary_json'],true)?:[];
    ...
    return ['available'=>true,'baseline_run_id'=>$b['public_id'],'regressed_metrics'=>$regressed,'deltas'=>$deltas];
}
...
function data_evaluation_export_run(PDO $pdo,array $viewer,string $runPublicId): array {
    data_evaluation_require_admin($viewer);$run=data_evaluation_run_get($pdo,$runPublicId);if(!$run)throw new RuntimeException('Evaluation run not found.');if($run['status']!=='completed')throw new RuntimeException('Only completed evaluation runs can be exported.');
    $q=$pdo->prepare('SELECT * FROM data_evaluation_suites WHERE id=?');$q->execute([$run['suite_id']]);$suite=$q->fetch();if(!$suite)throw new RuntimeException('Evaluation suite not found.');$results=data_evaluation_results($pdo,(int)$run['id']);$reviews=data_evaluation_reviews_for_run($pdo,(int)$run['id']);$integrity=data_evaluation_run_integrity($pdo,$run);
    $payload=['schema'=>'annotated.evaluation-run.v1','run'=>['public_id'=>$run['public_id'],'benchmark_type'=>$run['benchmark_type'],'status'=>$run['status'],'dataset_manifest_hash'=>$run['dataset_manifest_hash'],'suite_config_hash'=>$run['suite_config_hash'],'cases_hash'=>$run['cases_hash'],'model_snapshot'=>json_decode((string)($run['model_snapshot_json']??''),true),'summary'=>$run['summary'],'summary_hash'=>$run['result_summary_hash'],'run_hash'=>$run['run_hash'],'created_at'=>$run['created_at'],'started_at'=>$run['started_at'],'completed_at'=>$run['completed_at']],'suite'=>['public_id'=>$run['suite_public_id'],'name'=>$run['suite_name'],'config'=>json_decode((string)$suite['config_json'],true),'config_hash'=>$suite['config_hash']],'dataset'=>['public_id'=>$run['dataset_public_id'],'name'=>$run['dataset_name']],'integrity'=>$integrity,'results'=>[],'human_reviews'=>$reviews];
    foreach($results as $r)$payload['results'][]=['case_public_id'=>$r['case_public_id'],'label'=>$r['label'],'query_text'=>$r['query_text'],'reference_answer'=>$r['reference_answer'],'expected_corpus_public_id'=>$r['expected_corpus_public_id'],'rank_of_expected'=>$r['rank_of_expected'],'retrieved'=>$r['retrieved'],'response_text'=>$r['response_text'],'response_hash'=>$r['response_hash'],'citations'=>$r['citations'],'metrics'=>$r['metrics'],'metrics_hash'=>$r['metrics_hash'],'result_hash'=>$r['result_hash'],'passed'=>(int)$r['passed']];
    data_evaluation_event($pdo,(int)$run['suite_id'],(int)$run['id'],(int)$viewer['id'],'run_exported',['integrity_ok'=>$integrity['ok'],'run_hash'=>$run['run_hash']]);return $payload;
}
