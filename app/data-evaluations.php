...
function data_evaluation_execute_run(PDO $pdo,array $config,int $runId): array {
    ...
    try{
        ...
        foreach($cases as $case){
            $caseStarted=microtime(true);$ranked=data_evaluation_rank_rows($rows,(string)$case['query_text'],$topK);$expectedRank=null;foreach($ranked['all'] as $hit)if((int)$hit['dataset_item_id']===(int)$case['expected_dataset_item_id']){$expectedRank=(int)$hit['rank'];break;}
