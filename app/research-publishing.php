...
function research_publication_workflow_create(PDO $pdo,array $viewer,string $documentPublic,array $input=[]): array {
    ...
    $topics=function_exists('living_research_parse_topics')?living_research_parse_topics((array)($input['topics']??[])):[];$links=research_publication_source_links($pdo,(int)$doc['id']);$public=ulid_like();
...
function research_publication_required_approvals(PDO $pdo,array $w): array {
    if(empty($w['current_review_id']))return ['approved'=>0,'required'=>max(1,(int)$w['policy']['required_approvals']),'blocking'=>0,'assignments'=>[]];
    $q=$pdo->prepare("SELECT rra.reviewer_user_id,rra.reviewer_role,rra.is_required,rrr.decision,u.display_name,u.username FROM research_review_assignments rra JOIN users u ON u.id=rra.reviewer_user_id LEFT JOIN research_review_responses rrr ON rrr.id=rra.latest_response_id WHERE rra.review_id=? ORDER BY u.display_name,u.username");$q->execute([(int)$w['current_review_id']]);$rows=$q->fetchAll()?:[];$approved=0;$blocking=0;foreach($rows as $r){if(!(bool)$r['is_required'])continue;if($r['decision']==='approve')$approved++;elseif(in_array($r['decision'],['request_changes','disagree'],true))$blocking++;}
    return ['approved'=>$approved,'required'=>max(1,(int)$w['policy']['required_approvals']),'blocking'=>$blocking,'assignments'=>$rows];
}
...
function research_publication_publish(PDO $pdo,array $viewer,string $publicId): array {
    ...
    $pdo->prepare("UPDATE research_publication_workflows SET status='published',approval_snapshot_json=?,published_report_id=?,published_version_id=?,published_at=NOW(),updated_at=NOW() WHERE id=? AND status='approved'")->execute([json_encode($approval,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE),(int)$publish['report_id'],(int)$publish['version_id'],(int)$w['id']]);research_publication_event($pdo,(int)$w['id'],'published','user',(int)$viewer['id'],['report_public_id'=>$publish['public_id'],'version_number'=>$publish['version_number'],'snapshot_hash'=>$publish['snapshot_hash']]);$fresh=research_publication_workflow_access($pdo,$viewer,$publicId);$distribution=research_publication_distribute($pdo,$viewer,$fresh,$publish);return array_merge($publish,['distribution'=>$distribution,'workflow'=>$fresh]);
}
