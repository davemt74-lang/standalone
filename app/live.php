...
function live_presence_rows(PDO $pdo,array $viewer,array $room,string $viewerMode='cloaked'): array {
    ...
    foreach($q->fetchAll() as $r){
        ...
        $canSee=$subject===(int)$viewer['id']||presence_identity_visible($pdo,(int)$viewer['id'],$subject,(string)$r['presence_mode']);
...
function live_message_rows(PDO $pdo,array $viewer,array $room,int $afterId=0,int $limit=100): array {
    ...
    $out=[];$cursor=$afterId;
    foreach($q->fetchAll() as $m){$cursor=max($cursor,(int)$m['id']);$isSelf=(int)$m['user_id']===(int)$viewer['id'];$identityVisible=$isSelf||$m['identity_mode']==='visible'||($m['identity_mode']==='team_only'&&presence_identity_visible($pdo,(int)$viewer['id'],(int)$m['user_id'],'team_only'));$deleted=!empty($m['deleted_at']);
...
function live_event_emit_for_annotation(PDO $pdo,int $annotationId,string $eventType='annotation',?int $commentId=null): void {
    $q=$pdo->prepare('SELECT a.id,a.public_id,a.user_id,a.source_id,a.visibility,a.team_id FROM annotations a WHERE a.id=? LIMIT 1');$q->execute([$annotationId]);$a=$q->fetch();if(!$a)return;$objectId=(string)$a['public_id'];$suffix=$commentId?':'.$commentId:'';
    if($a['visibility']==='public')live_event_emit($pdo,(int)$a['source_id'],'public',null,null,(int)$a['user_id'],$eventType,'annotation',$objectId,[],$eventType.':'.$a['id'].$suffix.':public');
    elseif($a['visibility']==='team'&&!empty($a['team_id']))live_event_emit($pdo,(int)$a['source_id'],'team',(int)$a['team_id'],null,(int)$a['user_id'],$eventType,'annotation',$objectId,[],$eventType.':'.$a['id'].$suffix.':team:'.$a['team_id']);
    $q=$pdo->prepare('SELECT project_id FROM project_annotations WHERE annotation_id=?');$q->execute([$annotationId]);foreach($q->fetchAll(PDO::FETCH_COLUMN) as $projectId)live_event_emit($pdo,(int)$a['source_id'],'project',null,(int)$projectId,(int)$a['user_id'],$eventType,'annotation',$objectId,[],$eventType.':'.$a['id'].$suffix.':project:'.$projectId);
}
