...
function release_manifest_data(string $root,?string $buildSha=null): array {
    ...
    return [
      ...
      'channel'=>defined('ANNOTATED_RELEASE_CHANNEL')?ANNOTATED_RELEASE_CHANNEL:'stable',
...
function release_installed_manifest_status(string $root): array {
    ...
    $errors=[];if(($manifest['schema']??'')!=='annotated.release-manifest.v1')$errors[]='schema mismatch';if(($manifest['release']??'')!==ANNOTATED_RELEASE)$errors[]='release label mismatch';if(($manifest['version']??'')!==ANNOTATED_RELEASE_VERSION)$errors[]='application version mismatch';if((int)($manifest['phase']??0)!==ANNOTATED_RELEASE_PHASE)$errors[]='release phase mismatch';if(($manifest['channel']??'')!==(defined('ANNOTATED_RELEASE_CHANNEL')?ANNOTATED_RELEASE_CHANNEL:'stable'))$errors[]='release channel mismatch';if(($manifest['extension_version']??'')!==ANNOTATED_EXTENSION_VERSION)$errors[]='extension version mismatch';
