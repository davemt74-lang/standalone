...
function research_program_snapshot(PDO $pdo,array $program): array {
    ...
    if(installer_table_exists($pdo,'research_autonomy_observations')){$q=$pdo->prepare("SELECT public_id,fingerprint,severity,status,title,subject_public_id FROM research_autonomy_observations WHERE project_id=? AND observation_type='contradiction' ORDER BY id");$q->execute([$projectId]);foreach($q->fetchAll() as $r)$snapshot['contradictions'][(string)$r['fingerprint']]=['public_id'=>(string)$r['public_id'],'severity'=>(string)$r['severity'],'status'=>(string)$r['status'],'title'=>(string)$r['title'],'subject_public_id'=>(string)($r['subject_public_id']??'')];}
...
function research_program_memory(PDO $pdo,array $program,int $limit=12): array {
    ...
    return ['runs'=>$runs,'delta_types'=>$deltaTypes,'recurring_sources'=>$sources,'task_states'=>$taskStates,'text'=>implode(' ',$parts)];
}
...
function research_program_enqueue_due(PDO $pdo,int $limit=100): int {
    ...
    foreach($q->fetchAll(PDO::FETCH_COLUMN) as $id){$program=research_program_by_id($pdo,(int)$id);if(!$program)continue;$scheduled=(string)$program['next_run_at'];if(research_program_active_count($pdo,(int)$program['id'])>=(int)$program['max_concurrent_runs'])continue;
        if(research_program_month_count($pdo,(int)$program['id'])>=(int)$program['monthly_run_limit']){$zone=research_automation_timezone((string)$program['timezone_name']);$utc=new DateTimeZone('UTC');$local=(new DateTimeImmutable('first day of next month',$zone))->setTime((int)substr((string)$program['run_time_local'],0,2),(int)substr((string)$program['run_time_local'],3,2));$pdo->prepare("UPDATE research_programs SET next_run_at=?,updated_at=NOW() WHERE id=?")->execute([$local->setTimezone($utc)->format('Y-m-d H:i:s'),(int)$program['id']]);research_program_event($pdo,(int)$program['id'],(int)$program['project_id'],null,'monthly_limit_deferred','system',null,['monthly_run_limit'=>(int)$program['monthly_run_limit']]);continue;}
