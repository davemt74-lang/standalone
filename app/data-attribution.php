...
function data_object_descriptor(PDO $pdo,string $objectType,string $publicId): ?array {
    ...
    if($objectType==='model_improvement_example'){
        ...
        $meta=['case_public_id'=>$p['case_public_id'],'route_key'=>$p['route_key'],'model_version_label'=>$p['model_version_label'],'content_hash'=>$p['content_hash'],'source_material_policy'=>'human_sanitized_no_raw_production_text'];
...
function data_corpus_refresh_object(PDO $pdo,string $objectType,string $publicId): array {
    ...
    $pdo->prepare("UPDATE data_corpus_items SET invalidated_at=COALESCE(invalidated_at,NOW()),shared_retrieval_eligible=0,evaluation_eligible=0,training_eligible=0,commercial_training_eligible=0,eligibility_reason='superseded_object_version',refreshed_at=NOW() WHERE source_object_type=? AND source_object_public_id=? AND invalidated_at IS NULL AND COALESCE(source_object_version,'')<>?")->execute([$d['object_type'],$publicId,$version]);
