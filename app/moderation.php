...
function rights_claim_create(PDO $pdo,?array $viewer,string $annotationPublicId,string $name,string $email,string $claimType,string $description): array {
    ...
    $q=$pdo->prepare("SELECT id,user_id,public_id FROM annotations WHERE public_id=? AND visibility='public' AND status IN ('published','restricted') LIMIT 1");$q->execute([$annotationPublicId]);$a=$q->fetch();if(!$a)throw new RuntimeException('Annotation not found.');
