...
function intelligence_release_closed_loop_sample(PDO $pdo): array {
    ...
    $suiteOk=false;if($campaign['baseline_evaluation_suite_id']){$q=$pdo->prepare('SELECT status FROM data_evaluation_suites WHERE id=?');$q->execute([$campaign['baseline_evaluation_suite_id']]);$suiteStatus=(string)($q->fetchColumn()?:'');$suiteOk=in_array($suiteStatus,['draft','active'],true);}
    $checks[]=intelligence_release_audit_check('phase39_suite','Phase 39 regression handoff',$suiteOk,$suiteOk?'Campaign has a governed regression-suite handoff.':'Campaign regression suite is missing or invalid.');
