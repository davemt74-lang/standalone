...
function account_membership_expire_pending(PDO $pdo,int $accountId): int {
    if(!account_membership_ready($pdo))return 0;$q=$pdo->prepare("SELECT id FROM account_invitations WHERE account_id=? AND status='pending' AND expires_at<=NOW()");$q->execute([$accountId]);$ids=array_map('intval',$q->fetchAll(PDO::FETCH_COLUMN)?:[]);if(!$ids)return 0;$ownTx=!$pdo->inTransaction();if($ownTx)$pdo->beginTransaction();
