...
function account_membership_create_invitation(PDO $pdo,array $config,array $actor,string $accountPublicId,string $email,string $role='member',bool $reserveSeat=true,int $expiresDays=7,string $reason='Invite account member.'): array {
    ...
    $seed=account_admin_get($pdo,$accountPublicId);if(!$seed)throw new RuntimeException('Account not found.');account_membership_require_manage($pdo,$actor,$seed);if(PHP_SAPI!=='cli'&&function_exists('rate_limit_page_message')){$limited=rate_limit_page_message($pdo,'account-invite-create','user:'.(int)$actor['id'].':account:'.(int)$seed['id'],30,3600);if($limited!==null)throw new RuntimeException($limited);}$email=account_membership_email($email);$newRole=in_array($role,['admin','member'],true)?$role:'member';$reason=account_membership_reason($reason,'Invite account member.');$expiresDays=max(1,min(30,$expiresDays));
...
function account_membership_resend_invitation(PDO $pdo,array $config,array $actor,string $invitationPublicId,int $expiresDays=7,string $reason='Resend account invitation.'): array {
    $invite=account_membership_invitation_by_public($pdo,$invitationPublicId);if(!$invite)throw new RuntimeException('Invitation not found.');$account=account_admin_get($pdo,(int)$invite['account_id'])??throw new RuntimeException('Account not found.');account_membership_require_manage($pdo,$actor,$account);if(PHP_SAPI!=='cli'&&function_exists('rate_limit_page_message')){$limited=rate_limit_page_message($pdo,'account-invite-resend','user:'.(int)$actor['id'].':account:'.(int)$account['id'],60,3600);if($limited!==null)throw new RuntimeException($limited);}$reason=account_membership_reason($reason,'Resend account invitation.');$expiresDays=max(1,min(30,$expiresDays));
    return commercial_account_with_lock($pdo,(int)$account['id'],function()use($pdo,$config,$actor,$invite,$expiresDays,$reason){
        $account=account_admin_get($pdo,(int)$invite['account_id'])??throw new RuntimeException('Account not found.');account_membership_require_manage($pdo,$actor,$account);$fresh=account_membership_invitation_by_public($pdo,(string)$invite['public_id']);if(!$fresh||$fresh['status']!=='pending')throw new RuntimeException('Only pending invitations can be resent.');if($account['status']!=='active')throw new RuntimeException('Only active accounts can send invitations.');
...
function account_membership_revoke_invitation(PDO $pdo,array $actor,string $invitationPublicId,string $reason='Revoke account invitation.'): array {
    ...
    return commercial_account_with_lock($pdo,(int)$account['id'],function()use($pdo,$actor,$invite,$reason){
        $account=account_admin_get($pdo,(int)$invite['account_id'])??throw new RuntimeException('Account not found.');account_membership_require_manage($pdo,$actor,$account);$fresh=account_membership_invitation_by_public($pdo,(string)$invite['public_id']);if(!$fresh||$fresh['status']!=='pending')throw new RuntimeException('Only pending invitations can be revoked.');
...
function account_membership_remove_member(PDO $pdo,array $actor,string $accountPublicId,int $userId,string $reason='Remove account member.'): array {
    ...
    return commercial_account_with_lock($pdo,(int)$account['id'],function()use($pdo,$actor,$accountPublicId,$userId,$reason){
...
function account_membership_transfer_owner(PDO $pdo,array $actor,string $accountPublicId,int $newOwnerUserId,string $reason='Transfer account ownership.'): array {
    ...
    return commercial_account_with_lock($pdo,(int)$account['id'],function()use($pdo,$actor,$accountPublicId,$newOwnerUserId,$reason){
        $fresh=account_admin_get($pdo,$accountPublicId)??throw new RuntimeException('Account not found.');account_membership_require_owner($pdo,$actor,$fresh);$oldOwner=(int)$fresh['owner_user_id'];if($oldOwner===$newOwnerUserId)return $fresh;
