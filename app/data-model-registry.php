...
function data_model_benchmark_fingerprint(PDO $pdo,array $run): array {
    $suite=data_evaluation_suite_get($pdo,(string)$run['suite_public_id']);if(!$suite)throw new RuntimeException('Evaluation suite unavailable for model comparison.');$config=(array)$suite['config'];unset($config['model_id']);$payload=['dataset_manifest_hash'=>(string)$run['dataset_manifest_hash'],'cases_hash'=>(string)$run['cases_hash'],'benchmark_type'=>(string)$run['benchmark_type'],'top_k'=>(int)$suite['top_k'],'prompt_template'=>(string)($suite['prompt_template']??''),'config_without_model'=>$config];return ['hash'=>data_attribution_hash($payload),'payload'=>$payload,'suite'=>$suite];
}
