...
function data_model_version_create(PDO $pdo,array $viewer,string $registryPublicId,array $input): array {
    ...
    $policy=data_model_gate_policy_normalize($input);$policyJson=data_attribution_encode($policy);$policyHash=hash('sha256',$policyJson);$metadata=['registered_at'=>gmdate('c')];$public=ulid_like();
    $pdo->prepare("INSERT INTO data_model_versions(public_id,registry_id,ai_model_id,version_label,origin,status,architecture,intended_use,license_code,source_uri,context_window,artifact_ref,artifact_hash,metadata_json,gate_policy_json,gate_policy_hash,created_by_user_id) VALUES(?,?,?,?,?,'experimental',?,?,?,?,?,?,?,?,?,?,?)")
        ->execute([$public,$registry['id'],$aiModelId,$label,$origin,$architecture,$intended,$license,$source,$context,$artifact,$artifactHash,data_attribution_encode($metadata),$policyJson,$policyHash,$viewer['id']]);$id=(int)$pdo->lastInsertId();
...
function data_model_version_update_experimental(PDO $pdo,array $viewer,string $versionPublicId,array $input): array {
    ...
    $policy=data_model_gate_policy_normalize(array_merge((array)$v['gate_policy'],$input));$policyJson=data_attribution_encode($policy);$policyHash=hash('sha256',$policyJson);
    $pdo->prepare('UPDATE data_model_versions SET architecture=?,intended_use=?,license_code=?,source_uri=?,context_window=?,artifact_ref=?,artifact_hash=?,gate_policy_json=?,gate_policy_hash=?,updated_at=NOW() WHERE id=?')->execute([$architecture,$intended,$license,$source,$context,$artifact,$artifactHash,$policyJson,$policyHash,$v['id']]);data_model_event($pdo,(int)$v['registry_id'],(int)$v['id'],(int)$viewer['id'],'version_updated',['gate_policy_hash'=>$policyHash]);return data_model_version_get($pdo,$versionPublicId)??[];
}
function data_model_link_evaluation(PDO $pdo,array $viewer,string $versionPublicId,string $runPublicId,string $note=''): array {
    data_model_require_admin($viewer);$v=data_model_version_get($pdo,$versionPublicId);$run=data_evaluation_run_get($pdo,$runPublicId);if(!$v||!$run)throw new RuntimeException('Model version or evaluation run not found.');if($run['status']!=='completed')throw new RuntimeException('Only completed evaluation runs can be linked.');
...
function data_model_evidence_snapshot(PDO $pdo,array $version): array {
    $links=data_model_evaluation_links($pdo,(int)$version['id']);$runs=[];$validModelRuns=0;$totalReviews=0;$humanPass=0;$humanFail=0;$integrityFailures=0;$regressedMetrics=0;$suites=[];
    foreach($links as $link){$run=data_evaluation_run_get($pdo,(string)$link['run_public_id']);if(!$run)continue;$integrity=data_evaluation_run_integrity($pdo,$run);if(!$integrity['ok'])$integrityFailures++;$human=data_evaluation_human_summary($pdo,(int)$run['id']);$totalReviews+=$human['reviews'];$humanPass+=$human['pass'];$humanFail+=$human['fail'];$suite=data_evaluation_suite_get($pdo,(string)$run['suite_public_id']);$reg=$suite?data_evaluation_regression($pdo,$suite,$run):null;if($reg&&($reg['available']??false))$regressedMetrics+=max(0,(int)$reg['regressed_metrics']);if($run['benchmark_type']==='model'&&$integrity['ok']&&$version['ai_model_id']&&(int)$run['model_id']===(int)$version['ai_model_id'])$validModelRuns++;$suites[]=(string)$run['suite_public_id'];$runs[]=['public_id'=>$run['public_id'],'run_hash'=>$run['run_hash'],'suite_public_id'=>$run['suite_public_id'],'dataset_public_id'=>$run['dataset_public_id'],'dataset_manifest_hash'=>$run['dataset_manifest_hash'],'benchmark_type'=>$run['benchmark_type'],'model_id'=>$run['model_id']!==null?(int)$run['model_id']:null,'integrity_ok'=>$integrity['ok'],'summary_hash'=>$run['result_summary_hash'],'human_summary'=>$human,'regression'=>$reg];}
    $suites=array_values(array_unique($suites));sort($suites,SORT_STRING);$reviewed=$humanPass+$humanFail;$passRate=$reviewed>0?$humanPass/$reviewed:0.0;
    return ['version_public_id'=>$version['public_id'],'ai_model_id'=>$version['ai_model_id']!==null?(int)$version['ai_model_id']:null,'linked_runs'=>count($runs),'valid_model_runs'=>$validModelRuns,'integrity_failures'=>$integrityFailures,'human_reviews'=>$totalReviews,'human_pass'=>$humanPass,'human_fail'=>$humanFail,'human_pass_rate'=>$passRate,'regressed_metrics'=>$regressedMetrics,'suite_public_ids'=>$suites,'runs'=>$runs];
}
function data_model_gate_evaluate(PDO $pdo,array $version): array {
    ...
    $max=(int)$policy['max_regressed_metrics'];$checks['regression']=['pass'=>!(int)$policy['require_no_regression']||$evidence['regressed_metrics']<=$max,'actual'=>$evidence['regressed_metrics'],'required_max'=>$max];
    $missing=array_values(array_diff((array)$policy['required_suite_public_ids'],$evidence['suite_public_ids']));$checks['required_suites']=['pass'=>count($missing)===0,'missing'=>$missing,'required'=>(array)$policy['required_suite_public_ids']];
...
function data_model_receipt_create(PDO $pdo,array $viewer,array $version,string $from,string $to,?int $previousActiveId,array $gate,string $note=''): array {
    $gateJson=data_attribution_encode(['policy'=>$gate['policy'],'policy_hash'=>$gate['policy_hash'],'checks'=>$gate['checks'],'pass'=>$gate['pass'],'evaluated_at'=>$gate['evaluated_at']]);$gateHash=hash('sha256',$gateJson);$evidenceJson=data_attribution_encode($gate['evidence']);$evidenceHash=hash('sha256',$evidenceJson);$public=ulid_like();
    $core=['public_id'=>$public,'version_public_id'=>$version['public_id'],'from_status'=>$from,'to_status'=>$to,'previous_active_version_id'=>$previousActiveId,'gate_snapshot_hash'=>$gateHash,'evidence_snapshot_hash'=>$evidenceHash,'actor_user_id'=>(int)$viewer['id']];$receiptHash=data_attribution_hash($core);
...
function data_model_transition(PDO $pdo,array $viewer,string $versionPublicId,string $toStatus,string $note=''): array {
    ...
    $pdo->beginTransaction();try{
        ...
        if($toStatus==='active'){
            if($previousActive&&$previousActive!==(int)$version['id']){$q=$pdo->prepare("SELECT status FROM data_model_versions WHERE id=? FOR UPDATE");$q->execute([$previousActive]);$previousStatus=(string)$q->fetchColumn();if($previousStatus==='active'){$pdo->prepare("UPDATE data_model_versions SET status='deprecated',status_changed_at=NOW(),updated_at=NOW() WHERE id=?")->execute([$previousActive]);data_model_event($pdo,(int)$version['registry_id'],$previousActive,(int)$viewer['id'],'superseded_by_activation',['new_active_version_public_id'=>$versionPublicId]);}}
...
function data_model_rollback(PDO $pdo,array $viewer,string $registryPublicId,string $targetVersionPublicId,string $note=''): array {
    ...
    $pdo->beginTransaction();try{
        ...
        if($current){$pdo->prepare("UPDATE data_model_versions SET status='deprecated',status_changed_at=NOW(),updated_at=NOW() WHERE id=? AND status='active'")->execute([$current]);data_model_event($pdo,(int)$registry['id'],$current,(int)$viewer['id'],'superseded_by_rollback',['target_version_public_id'=>$targetVersionPublicId]);}
...
function data_model_comparison(PDO $pdo,array $versionPublicIds): array {
    ...
    $rows=[];foreach($ids as $id){$v=data_model_version_get($pdo,$id);if(!$v)throw new RuntimeException('Model version not found: '.$id);$gate=data_model_gate_evaluate($pdo,$v);$rows[]=['version'=>$v,'gate'=>$gate,'links'=>data_model_evaluation_links($pdo,(int)$v['id'])];}
    return ['versions'=>$rows,'generated_at'=>gmdate('c')];
}
