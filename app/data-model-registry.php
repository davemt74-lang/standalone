...
function data_model_comparison(PDO $pdo,array $versionPublicIds): array {
    ...
    $rows=[];$suiteSets=[];foreach($ids as $id){$v=data_model_version_get($pdo,$id);if(!$v)throw new RuntimeException('Model version not found: '.$id);$gate=data_model_gate_evaluate($pdo,$v);$links=data_model_evaluation_links($pdo,(int)$v['id']);$valid=[];foreach($links as $link){$run=data_evaluation_run_get($pdo,(string)$link['run_public_id']);if(!$run||$run['benchmark_type']!=='model'||(int)$run['model_id']!==(int)$v['ai_model_id'])continue;$integrity=data_evaluation_run_integrity($pdo,$run);if(!$integrity['ok'])continue;$valid[(string)$run['suite_public_id']][]=$run;}$suiteSets[]=array_keys($valid);$rows[]=['version'=>$v,'gate'=>$gate,'valid_runs_by_suite'=>$valid];}
    $common=$suiteSets?array_shift($suiteSets):[];foreach($suiteSets as $set)$common=array_values(array_intersect($common,$set));sort($common,SORT_STRING);$comparisons=[];
    foreach($common as $suitePublic){$suite=data_evaluation_suite_get($pdo,$suitePublic);$entry=['suite_public_id'=>$suitePublic,'suite_name'=>$suite['name']??$suitePublic,'dataset_public_id'=>$suite['dataset_public_id']??null,'dataset_name'=>$suite['dataset_name']??null,'versions'=>[]];foreach($rows as $row){$runs=$row['valid_runs_by_suite'][$suitePublic]??[];usort($runs,fn($a,$b)=>(int)$b['id']<=>(int)$a['id']);$run=$runs[0]??null;$entry['versions'][]=['version'=>$row['version'],'run'=>$run,'summary'=>$run['summary']??null,'human'=>$run?data_evaluation_human_summary($pdo,(int)$run['id']):null];}$comparisons[]=$entry;}
    foreach($rows as &$row)unset($row['valid_runs_by_suite']);unset($row);return ['versions'=>$rows,'common_suites'=>$comparisons,'generated_at'=>gmdate('c')];
}
