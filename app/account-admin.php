...
function account_admin_change_package(PDO $pdo,array $admin,string $accountPublicId,string $packagePublicId,string $reason): array {
    ...
    return commercial_account_with_lock($pdo,(int)$seed['id'],function()use($pdo,$admin,$accountPublicId,$userIdentifier,$role,$reason){
        $account=account_admin_get($pdo,$accountPublicId);if(!$account)throw new RuntimeException('Account not found.');if($account['status']==='closed')throw new RuntimeException('Closed accounts cannot accept members.');
        ...
        $pdo->beginTransaction();try{$pdo->prepare('INSERT INTO account_members(account_id,user_id,account_role) VALUES(?,?,?)')->execute([(int)$account['id'],(int)$user['id'],$newRole]);account_admin_event($pdo,(int)$account['id'],(int)$admin['id'],(int)$user['id'],'member_added',null,['account_role'=>$newRole],$reason);$pdo->commit();return account_admin_members($pdo,(int)$account['id']);}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
    });
    ...
    return commercial_account_with_lock($pdo,(int)$seed['id'],function()use($pdo,$admin,$accountPublicId,$key,$value,$expiresAt,$reason){
        ...
        if($entitlementKey==='member_limit'){$countQ=$pdo->prepare('SELECT COUNT(*) FROM account_members WHERE account_id=?');$countQ->execute([(int)$account['id']]);if((int)$normalized<(int)$countQ->fetchColumn())throw new RuntimeException('Member limit cannot be set below the current member count.');}
