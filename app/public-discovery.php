...
function public_discovery_annotation(PDO $pdo,string $publicId,?array $viewer): ?array {
    ...
    $a['context_url']=feed_context_url($a);$a['source_changed']=(int)$a['source_version_id']!==(int)$a['current_version_id'];$a['integrity']=source_integrity_annotation_state($pdo,$a);
...
function public_discovery_source(PDO $pdo,string $publicId,?array $viewer): ?array {
    ...
    $source['events']=[];try{$source['events']=source_integrity_source_timeline($pdo,(int)$source['id'],30);}catch(PDOException $e){}
