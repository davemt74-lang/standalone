...
function public_discovery_annotation(PDO $pdo,string $publicId,?array $viewer): ?array {
    $access=annotation_access($pdo,$publicId,$viewer);if(!$access||$access['status']!=='published')return null;$uid=(int)($viewer['id']??0);
...
function public_discovery_profile(PDO $pdo,string $username,?array $viewer): ?array {
    ...
    $owner=$viewer&&(int)$viewer['id']===(int)$p['id'];if($p['profile_visibility']!=='public'&&!$owner)return null;
...
function public_discovery_explore(PDO $pdo): array {
    $sources=$pdo->query("SELECT s.public_id,s.title,s.domain,s.canonical_url,s.status,COUNT(a.id) annotation_count,COUNT(DISTINCT a.user_id) contributor_count,COUNT(DISTINCT c.id) comment_count,MAX(a.published_at) last_activity FROM sources s JOIN annotations a ON a.source_id=s.id AND a.visibility='public' AND a.status='published' LEFT JOIN comments c ON c.annotation_id=a.id GROUP BY s.id ORDER BY last_activity DESC LIMIT 24")->fetchAll();
    ...
    $annotations=$pdo->query("SELECT a.public_id,a.text_commentary,a.published_at,u.username,u.display_name,s.public_id source_public_id,s.title source_title,s.domain,c.selected_text,c.capture_type,c.media_provider,c.media_title,(SELECT COUNT(*) FROM comments cm WHERE cm.annotation_id=a.id) comment_count FROM annotations a JOIN users u ON u.id=a.user_id JOIN sources s ON s.id=a.source_id JOIN captures c ON c.id=a.capture_id WHERE a.visibility='public' AND a.status='published' ORDER BY a.id DESC LIMIT 24")->fetchAll();
    $media=$pdo->query("SELECT a.public_id,a.text_commentary,a.published_at,u.username,u.display_name,s.public_id source_public_id,s.title source_title,c.capture_type,c.media_provider,c.media_title,c.media_author,md.processing_status FROM annotations a JOIN users u ON u.id=a.user_id JOIN sources s ON s.id=a.source_id JOIN captures c ON c.id=a.capture_id LEFT JOIN media_derivatives md ON md.capture_id=c.id WHERE a.visibility='public' AND a.status='published' AND c.capture_type IN ('video_clip','audio_clip','image_region','page_region') ORDER BY a.id DESC LIMIT 12")->fetchAll();
    $reports=$pdo->query("SELECT rr.public_id,rr.title,rr.summary,rr.published_at,rv.version_number,u.username,u.display_name FROM research_reports rr JOIN research_report_versions rv ON rv.id=rr.current_version_id JOIN users u ON u.id=rr.created_by_user_id WHERE rr.visibility='public' AND rr.status='published' ORDER BY rr.published_at DESC LIMIT 12")->fetchAll();
    return compact('sources','discussed','annotations','media','reports');
}
function public_discovery_search(PDO $pdo,string $term): array {
    $out=['sources'=>[],'annotations'=>[],'people'=>[],'reports'=>[]];if(mb_strlen($term)<2)return $out;$like='%'.$term.'%';
    ...
    $q=$pdo->prepare("SELECT DISTINCT a.public_id,a.text_commentary,a.published_at,u.username,u.display_name,s.public_id source_public_id,s.title source_title,c.selected_text,c.capture_type,COALESCE(t.edited_text,t.raw_text) transcript_text FROM annotations a JOIN users u ON u.id=a.user_id JOIN sources s ON s.id=a.source_id JOIN captures c ON c.id=a.capture_id LEFT JOIN annotation_transcripts t ON t.annotation_id=a.id WHERE a.visibility='public' AND a.status='published' AND (a.text_commentary LIKE ? OR c.selected_text LIKE ? OR t.raw_text LIKE ? OR t.edited_text LIKE ? OR s.title LIKE ?) ORDER BY a.id DESC LIMIT 30");$q->execute([$like,$like,$like,$like,$like]);$out['annotations']=$q->fetchAll();
    $q=$pdo->prepare("SELECT u.username,u.display_name,u.bio FROM users u LEFT JOIN user_preferences p ON p.user_id=u.id WHERE u.status='active' AND COALESCE(p.profile_visibility,'public')='public' AND COALESCE(p.search_visibility,1)=1 AND (u.username LIKE ? OR u.display_name LIKE ? OR u.bio LIKE ?) ORDER BY u.display_name LIMIT 20");$q->execute([$like,$like,$like]);$out['people']=$q->fetchAll();
    $q=$pdo->prepare("SELECT rr.public_id,rr.title,rr.summary,rr.published_at,rv.version_number,rv.snapshot_json,u.username,u.display_name FROM research_reports rr JOIN research_report_versions rv ON rv.id=rr.current_version_id JOIN users u ON u.id=rr.created_by_user_id WHERE rr.visibility='public' AND rr.status='published' AND (rr.title LIKE ? OR rr.summary LIKE ? OR rv.snapshot_json LIKE ?) ORDER BY rr.published_at DESC LIMIT 20");$q->execute([$like,$like,$like]);$reports=$q->fetchAll();foreach($reports as &$r){$snapshot=json_decode((string)$r['snapshot_json'],true);$matches=[];if(is_array($snapshot)){foreach(($snapshot['findings']??[]) as $f)if(stripos((string)($f['title'].' '.$f['summary']),$term)!==false)$matches[]='Finding: '.($f['title']??'');foreach(($snapshot['claims']??[]) as $c)if(stripos((string)($c['statement']??''),$term)!==false)$matches[]='Claim: '.($c['statement']??'');foreach(($snapshot['entities']??[]) as $e)if(stripos((string)(($e['name']??'').' '.($e['description']??'')),$term)!==false)$matches[]='Entity: '.($e['name']??'');}$r['matches']=array_slice($matches,0,3);unset($r['snapshot_json']);}unset($r);$out['reports']=$reports;
    return $out;
}
