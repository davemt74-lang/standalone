...
function data_model_campaign_lock(PDO $pdo,array $viewer,string $campaignPublicId): array {
    ...
    $locked=data_model_campaign_locked($pdo,$campaignPublicId,function(array $campaign) use($pdo,$viewer,$campaignPublicId){
        ...
        $pdo->beginTransaction();try{
            $q=$pdo->prepare('SELECT c.id FROM data_model_improvement_campaign_cases cc JOIN data_model_improvement_cases c ON c.id=cc.improvement_case_id WHERE cc.campaign_id=? ORDER BY c.id FOR UPDATE');$q->execute([$campaign['id']]);$q->fetchAll();
            $q=$pdo->prepare('SELECT p.id FROM data_model_improvement_campaign_proposals cp JOIN data_model_improvement_proposals p ON p.id=cp.proposal_id WHERE cp.campaign_id=? ORDER BY p.id FOR UPDATE');$q->execute([$campaign['id']]);$q->fetchAll();
            $cases=data_model_campaign_cases($pdo,(int)$campaign['id']);$props=data_model_campaign_proposals($pdo,(int)$campaign['id']);if(!$cases)throw new RuntimeException('Campaign requires at least one human-triaged Phase 46 improvement case.');
            foreach($cases as $case)if(!data_model_improvement_case_actionable($case))throw new RuntimeException('Campaign cannot lock because a linked Phase 46 case is no longer actionable: '.$case['case_public_id']);
            $eval=array_values(array_filter($props,fn($p)=>$p['proposal_type']==='evaluation_case'));$train=array_values(array_filter($props,fn($p)=>$p['proposal_type']==='training_example'));if(!$eval)throw new RuntimeException('Campaign requires at least one published production regression proposal.');if($campaign['strategy']==='model_training'&&!$train)throw new RuntimeException('Controlled model-training campaign requires at least one approved training example.');
...
function data_model_campaign_add_case(PDO $pdo,array $viewer,string $campaignPublicId,string $casePublicId): array {
    ...
    return data_model_campaign_locked($pdo,$campaignPublicId,function(array $campaign) use($pdo,$viewer,$campaignPublicId,$casePublicId){
        ...
        $pdo->beginTransaction();try{
            $q=$pdo->prepare('INSERT IGNORE INTO data_model_improvement_campaign_cases(campaign_id,improvement_case_id,baseline_recurrence_count,baseline_evidence_count,baseline_evidence_hash,baseline_incident_metric,baseline_incident_value,baseline_incident_threshold) VALUES(?,?,?,?,?,?,?,?)');$q->execute([$campaign['id'],$case['id'],$case['recurrence_count'],$case['evidence_count'],$case['evidence_hash'],$metric,$value,$threshold]);
            if($q->rowCount()===1)data_model_campaign_event($pdo,(int)$campaign['id'],(int)$viewer['id'],'case_linked',['case_public_id'=>$case['public_id'],'evidence_hash'=>$case['evidence_hash']]);$pdo->commit();
        }catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
        return data_model_campaign_get($pdo,$campaignPublicId,false)??[];
    });
}
function data_model_campaign_remove_case(PDO $pdo,array $viewer,string $campaignPublicId,string $casePublicId): array {
    ...
    return data_model_campaign_locked($pdo,$campaignPublicId,function(array $campaign) use($pdo,$viewer,$campaignPublicId,$casePublicId){
        ...
        $pdo->beginTransaction();try{
            $deleted=0;if($id){$pdo->prepare('DELETE FROM data_model_improvement_campaign_proposals WHERE campaign_id=? AND proposal_id IN (SELECT id FROM data_model_improvement_proposals WHERE case_id=?)')->execute([$campaign['id'],$id]);$q=$pdo->prepare('DELETE FROM data_model_improvement_campaign_cases WHERE campaign_id=? AND improvement_case_id=?');$q->execute([$campaign['id'],$id]);$deleted=$q->rowCount();}
            if($deleted===1)data_model_campaign_event($pdo,(int)$campaign['id'],(int)$viewer['id'],'case_unlinked',['case_public_id'=>$casePublicId]);$pdo->commit();
        }catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
        return data_model_campaign_get($pdo,$campaignPublicId,false)??[];
    });
}
function data_model_campaign_add_proposal(PDO $pdo,array $viewer,string $campaignPublicId,string $proposalPublicId): array {
    ...
    return data_model_campaign_locked($pdo,$campaignPublicId,function(array $campaign) use($pdo,$viewer,$campaignPublicId,$proposalPublicId){
        if($campaign['plan_hash'])throw new RuntimeException('Locked campaign scope is immutable.');
        $p=data_model_improvement_proposal_locked($pdo,$proposalPublicId,function(array $p) use($pdo,$campaign,$proposalPublicId){
            if($p['status']!=='published'||empty($p['approval_hash'])||empty($p['corpus_item_public_id']))throw new RuntimeException('Campaigns may use only published, human-approved Phase 46 proposals.');
        ...
        $pdo->beginTransaction();try{
            $q=$pdo->prepare('INSERT IGNORE INTO data_model_improvement_campaign_proposals(campaign_id,proposal_id,proposal_type,content_hash,approval_hash,corpus_item_public_id) VALUES(?,?,?,?,?,?)');$q->execute([$campaign['id'],$p['id'],$p['proposal_type'],$p['content_hash'],$p['approval_hash'],$p['corpus_item_public_id']]);
            if($q->rowCount()===1)data_model_campaign_event($pdo,(int)$campaign['id'],(int)$viewer['id'],'proposal_linked',['proposal_public_id'=>$p['public_id'],'proposal_type'=>$p['proposal_type'],'content_hash'=>$p['content_hash'],'approval_hash'=>$p['approval_hash']]);$pdo->commit();
        }catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
        return data_model_campaign_get($pdo,$campaignPublicId,false)??[];
    });
}
function data_model_campaign_remove_proposal(PDO $pdo,array $viewer,string $campaignPublicId,string $proposalPublicId): array {
    ...
    return data_model_campaign_locked($pdo,$campaignPublicId,function(array $campaign) use($pdo,$viewer,$campaignPublicId,$proposalPublicId){
        ...
        $pdo->beginTransaction();try{
            $deleted=0;if($id){$q=$pdo->prepare('DELETE FROM data_model_improvement_campaign_proposals WHERE campaign_id=? AND proposal_id=?');$q->execute([$campaign['id'],$id]);$deleted=$q->rowCount();}
            if($deleted===1)data_model_campaign_event($pdo,(int)$campaign['id'],(int)$viewer['id'],'proposal_unlinked',['proposal_public_id'=>$proposalPublicId]);$pdo->commit();
        }catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
        return data_model_campaign_get($pdo,$campaignPublicId,false)??[];
    });
}
...
function data_model_campaign_create_dataset_drafts(PDO $pdo,array $viewer,string $campaignPublicId): array {
    ...
    return data_model_campaign_locked($pdo,$campaignPublicId,function(array $campaign) use($pdo,$viewer,$campaignPublicId){
        ...
        $pdo->beginTransaction();try{
            if(!$campaign['evaluation_dataset_id']){$d=data_dataset_create($pdo,$viewer,['name'=>'Campaign '.$campaign['title'].' · Regression','description'=>'Phase 47 campaign-specific regression dataset draft. Human freeze required.','purpose'=>'evaluation','corpus_types'=>['model_regression_case'],'source_object_public_ids'=>$eval,'max_items'=>count($eval)]);$q=$pdo->prepare("UPDATE data_model_improvement_campaigns SET evaluation_dataset_id=?,status='dataset_preparation',updated_at=NOW() WHERE id=? AND evaluation_dataset_id IS NULL AND plan_hash IS NULL");$q->execute([$d['id'],$campaign['id']]);if($q->rowCount()!==1)throw new RuntimeException('Campaign changed while linking evaluation dataset.');data_model_campaign_event($pdo,(int)$campaign['id'],(int)$viewer['id'],'evaluation_dataset_draft_created',['dataset_public_id'=>$d['public_id'],'selection_policy_hash'=>$d['selection_policy_hash'],'proposal_count'=>count($eval)]);}
            if($campaign['strategy']==='model_training'&&!$campaign['training_dataset_id']){$d=data_dataset_create($pdo,$viewer,['name'=>'Campaign '.$campaign['title'].' · Training','description'=>'Phase 47 campaign-specific approved training dataset draft. Human freeze required.','purpose'=>'training','corpus_types'=>['model_training_example'],'source_object_public_ids'=>$train,'max_items'=>count($train)]);$q=$pdo->prepare("UPDATE data_model_improvement_campaigns SET training_dataset_id=?,status='dataset_preparation',updated_at=NOW() WHERE id=? AND training_dataset_id IS NULL AND plan_hash IS NULL");$q->execute([$d['id'],$campaign['id']]);if($q->rowCount()!==1)throw new RuntimeException('Campaign changed while linking training dataset.');data_model_campaign_event($pdo,(int)$campaign['id'],(int)$viewer['id'],'training_dataset_draft_created',['dataset_public_id'=>$d['public_id'],'selection_policy_hash'=>$d['selection_policy_hash'],'proposal_count'=>count($train)]);}
            $pdo->commit();
        }catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
        return data_model_campaign_get($pdo,$campaignPublicId,false)??[];
    });
...
function data_model_campaign_current_use(PDO $pdo,array $campaign): array {
    ...
    $cases=data_model_campaign_cases($pdo,(int)$campaign['id']);$planCases=[];foreach((array)($plan['cases']??[]) as $pc)$planCases[(string)($pc['case_public_id']??'')]=$pc;$casePass=count($cases)===count($planCases);foreach($cases as $case){$pc=$planCases[(string)$case['case_public_id']]??null;if(!$pc||!data_model_improvement_case_actionable($case)||(string)$pc['classification']!==(string)$case['classification']||(string)$pc['route_key']!==(string)$case['route_key']||(string)$pc['baseline_evidence_hash']!==(string)$case['baseline_evidence_hash']){$casePass=false;break;}}$checks['case_governance']=['pass'=>$casePass,'count'=>count($cases)];
