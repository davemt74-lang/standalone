...
function research_network_cognitive_observations(PDO $pdo,array $viewer,array &$items,int $limitProjects=12): void {
    if(!research_network_ready($pdo))return;$q=$pdo->prepare("SELECT rp.public_id FROM research_projects rp WHERE rp.status='active' AND (rp.owner_user_id=? OR EXISTS(SELECT 1 FROM team_members tm WHERE tm.team_id=rp.team_id AND tm.user_id=?)) ORDER BY rp.updated_at DESC,rp.id DESC LIMIT ".$limitProjects);$q->execute([$viewer['id'],$viewer['id']]);
