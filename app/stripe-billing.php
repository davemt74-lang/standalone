...
function stripe_billing_create_checkout_for_account(PDO $pdo,array $config,array $user,array $account,string $packagePublicId,?string $promotionCode=null): array {
    ...
    return commercial_account_with_lock($pdo,(int)$account['id'],function()use($pdo,$config,$user,$account,$packagePublicId,$promotionCode){
        ...
        if(function_exists('commercial_billing_ready')&&commercial_billing_ready($pdo)){try{commercial_billing_sync_profile_to_stripe($pdo,$config,$fresh,commercial_billing_profile($pdo,(int)$fresh['id']));}catch(Throwable $e){}}
        ...
        $desiredPromotionPublic=$promotion?(string)$promotion['public_id']:'';$billingPolicyHash=function_exists('commercial_billing_ready')&&commercial_billing_ready($pdo)?commercial_billing_checkout_policy_hash($pdo):'';
        ...
        $storedBillingPolicy=function(array $row): string {$stored=json_decode((string)($row['request_json']??''),true);return is_array($stored)?(string)($stored['metadata']['annotated_billing_policy_hash']??''):'';};
        $pending=stripe_billing_checkout_pending($pdo,(int)$fresh['id'],$mode);$attemptPublic='';
        if($pending){
            $promoMatches=$storedPromotion($pending)===$desiredPromotionPublic;$billingPolicyMatches=$storedBillingPolicy($pending)===$billingPolicyHash;
            if($pending['status']==='open'){
                $notExpired=empty($pending['expires_at'])||strtotime((string)$pending['expires_at'])>time();
                if((int)$pending['package_id']===(int)$package['id']&&$promoMatches&&$billingPolicyMatches&&$notExpired&&!empty($pending['checkout_url'])){if(function_exists('commercial_promotion_bind_session')&&!empty($pending['stripe_checkout_session_id']))commercial_promotion_bind_session($pdo,$mode,(string)$pending['public_id'],(string)$pending['stripe_checkout_session_id']);return ['id'=>$pending['stripe_checkout_session_id'],'url'=>stripe_billing_hosted_url((string)$pending['checkout_url']),'status'=>'open','reused'=>true];}
                ...
                if((int)$pending['package_id']===(int)$package['id']&&$promoMatches&&$billingPolicyMatches)return ['id'=>$recoveredId,'url'=>$recoveredUrl,'status'=>'open','reused'=>true];
        ...
        $metadata=['annotated_account_id'=>(string)$fresh['public_id'],'annotated_package_id'=>(string)$package['public_id'],'requested_by_user_id'=>(string)$user['public_id'],'annotated_checkout_attempt'=>$attemptPublic,'annotated_billing_policy_hash'=>$billingPolicyHash];
        ...
        if(function_exists('commercial_billing_ready')&&commercial_billing_ready($pdo))$params=array_replace_recursive($params,commercial_billing_checkout_params($pdo,$fresh));
...
function stripe_billing_process_webhook(PDO $pdo,array $config,string $payload,string $signatureHeader): array {
    ...
    try{
        if($eventType==='checkout.session.completed'){
            ...
            if(function_exists('commercial_billing_prepare_draft_invoice')){$object=commercial_billing_prepare_draft_invoice($pdo,$config,$object,$mode);}stripe_billing_sync_invoice($pdo,$object,$eventType,$mode,$eventId,$eventCreatedAt);if(function_exists('commercial_billing_handle_invoice'))commercial_billing_handle_invoice($pdo,$object,$eventType,$mode,$eventId);
            ...
            stripe_billing_sync_invoice($pdo,$object,$eventType,$mode,$eventId,$eventCreatedAt);if(function_exists('ai_overage_handle_stripe_invoice'))ai_overage_handle_stripe_invoice($pdo,$object,$mode);if(function_exists('commercial_promotion_handle_invoice'))commercial_promotion_handle_invoice($pdo,$object,$mode);if(function_exists('commercial_billing_handle_invoice'))commercial_billing_handle_invoice($pdo,$object,$eventType,$mode,$eventId);
