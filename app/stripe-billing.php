...
function stripe_billing_sync_subscription(PDO $pdo,array $object,string $mode,?string $eventId=null,?string $eventCreatedAt=null): ?array {
    ...
    if($priceMap&&(int)$priceMap['package_id']!==(int)$account['package_id']){$targetPackage=subscription_package($pdo,(int)$priceMap['package_id']);if(!$targetPackage)throw new RuntimeException('Mapped Annotated package is unavailable.');stripe_billing_checkout_member_limit($pdo,$account,$targetPackage);}$periodStart=stripe_billing_datetime($object['current_period_start']??$first['current_period_start']??null);$periodEnd=stripe_billing_datetime($object['current_period_end']??$first['current_period_end']??null);$trialEnd=stripe_billing_datetime($object['trial_end']??null);
    $pdo->beginTransaction();try{
