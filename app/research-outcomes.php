...
function research_outcome_try_record(PDO $pdo,array $viewer,array $data): bool {
    try{$row=research_outcome_record($pdo,$viewer,$data);return !empty($row);}catch(RuntimeException|InvalidArgumentException $e){return false;}
}
...
function research_outcome_sync_agent_actions(PDO $pdo,array $viewer,int $limit=300): int {
    ...
    foreach($q->fetchAll() as $p){$decision=(string)$p['status'];$title=match($decision){'executed'=>'Agent proposal executed','rejected'=>'Agent proposal rejected','stale'=>'Agent proposal became stale',default=>'Agent proposal failed'};$refs=[['type'=>'agent_action','public_id'=>$p['public_id'],'role'=>'source']];if(!empty($p['result_type'])&&!empty($p['result_public_id']))$refs[]=['type'=>(string)$p['result_type'],'public_id'=>(string)$p['result_public_id'],'role'=>'result'];
        if(!project_access($pdo,(int)$viewer['id'],(string)$p['project_public_id']))continue;if(!research_outcome_try_record($pdo,$viewer,['event_type'=>'agent_action','decision_type'=>$decision,'source_type'=>'agent_action','source_public_id'=>$p['public_id'],'project_public_id'=>$p['project_public_id'],'object_type'=>'agent_action','object_public_id'=>$p['public_id'],'result_type'=>$p['result_type'],'result_public_id'=>$p['result_public_id'],'title'=>$title,'summary'=>'Capability: '.(string)$p['capability_key'],'metadata'=>['capability_key'=>$p['capability_key'],'status'=>$p['status']],'refs'=>$refs,'occurred_at'=>$p['updated_at'],'dedupe_key'=>'agent_action:'.$p['public_id'].':'.$p['status']]))continue;$n++;
    }return $n;
}
...
function research_outcome_sync_automations(PDO $pdo,array $viewer,int $limit=300): int {
    ...
    foreach($q->fetchAll() as $r){if(!project_access($pdo,(int)$viewer['id'],(string)$r['project_public_id']))continue;$decision=(string)$r['status'];if(!research_outcome_try_record($pdo,$viewer,['event_type'=>'automation_run','decision_type'=>$decision,'source_type'=>'automation_run','source_public_id'=>$r['public_id'],'project_public_id'=>$r['project_public_id'],'object_type'=>'automation','object_public_id'=>$r['automation_public_id'],'title'=>'Automation '.ucfirst($decision).' · '.$r['automation_title'],'summary'=>mb_substr((string)($r['output_text']?:$r['last_error']?:''),0,1200),'metadata'=>['proposal_count'=>(int)$r['proposal_count'],'trigger_type'=>$r['trigger_type']],'refs'=>[['type'=>'automation','public_id'=>$r['automation_public_id'],'role'=>'source'],['type'=>'automation_run','public_id'=>$r['public_id'],'role'=>'result']],'occurred_at'=>$r['completed_at']?:$r['created_at'],'dedupe_key'=>'automation_run:'.$r['public_id'].':'.$r['status']]))continue;$n++;}return $n;
}
