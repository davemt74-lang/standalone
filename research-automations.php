...
document.querySelectorAll('[data-automation-form]').forEach(form=>{
 ...
 const sync=()=>{const watchMode=trigger?.value==='watch_alert';if(schedule)schedule.hidden=watchMode;if(watch)watch.hidden=!watchMode;if(prompt)prompt.hidden=['source_refresh','cross_research_review','outcome_review'].includes(workflow?.value);};
